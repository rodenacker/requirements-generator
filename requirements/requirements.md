...
**Domain:** DevOps / Platform-as-a-Service for technical business users **Created:** 2026-04-30 **Status:** final **Last finalised at:** 2026-04-30

> Inferred content was reconciled with the consultant during Q&A; this document reflects the agreed final values.
...
**Domain:** DevOps / Platform-as-a-Service for technical business users
...
| Notification | persistent | An asynchronous-operation completion notification delivered to the invoking user. |
...
- User **receives** Notification for asynchronous operations they invoked [1:0..*]
...
| Expertise level | High — comfortable with platform/infrastructure concepts; typically the consulting company's internal IT lead or the on-premise customer's platform owner. |
...
| Driving forces — wants | Reliable tenant onboarding, clear visibility of tenant status, clean separation from tenant-internal data, full audit trail. |
| Driving forces — fears | Accidentally accessing tenant-internal data; deleting an active tenant; leaving the platform without an admin (PADM-08). |

### Tenant Administrator

| Field | Value |
| --- | --- |
| Role / job title | Tenant Administrator — typically a consulting-company partner, delivery lead, or operations manager. |
| Expertise level | Medium-high — comfortable with org/role concepts, OAuth/SSO providers, and project structure; not a DevOps specialist. |
...
| Driving forces — wants | Quick user onboarding, clear directory of who has access to what, ability to enforce SSO-only login, confidence that membership changes are auditable. |
| Driving forces — fears | Letting a former employee retain access; locking the tenant out by removing the last admin; accidentally deleting a project with live resources. |

### Project Administrator

| Field | Value |
| --- | --- |
| Role / job title | Project Administrator — the lead for a specific client engagement. |
| Expertise level | Medium-high — technically literate business user; understands environments, Git providers, role-based access, but not container orchestration. |
| Stakes | High — owns environment topology, project-level integrations, and member roles for the engagement. |
| Frequency of use | Medium — daily project dashboard checks; environment and membership changes a few times per week. |
| Driving forces — wants | A clear project dashboard, simple environment management, reliable Git integration, easy assignment of operators and viewers. |
| Driving forces — fears | Losing track of which member has which role across environments; misconfigured Git credentials breaking builds; deleting an environment that still has resources. |
...
| Stakes | High in the moment — incorrect deploys or stopped instances directly affect running applications and clients. |
| Frequency of use | Very high — multiple times per day; this persona generates the majority of portal traffic. |
| Driving forces — wants | Fastest path to deploy, fastest path to logs, immediate visibility of instance health, confidence that rollback is one click away. |
| Driving forces — fears | Deploying a broken build to production; losing log context during an incident; accidentally deleting an instance, database, or bucket; not noticing a degraded instance. |

### Viewer

| Field | Value |
| --- | --- |
| Role / job title | Viewer — read-only stakeholder (often the client, a QA engineer, or a non-technical project lead). |
| Expertise level | Low-medium — wants to see status without having to interpret infrastructure terms. |
| Stakes | Low — cannot change anything; risk is misreading status. |
| Frequency of use | Very high for dashboard/log viewing during demos, releases, and support windows. |
| Driving forces — wants | A clear at-a-glance picture of project health, easy navigation to logs and dashboards, masked secrets so they can be shown freely. |
| Driving forces — fears | Misreporting status to stakeholders; accidentally trying an action they don't have rights for. |
...
| --- | --- | --- | --- | --- | --- |
| G-01 | Sign in to the portal and reach my last-used tenant and project | low-friction; SSO-first; respects last-used context (USR-10) | top-level | enterprise console shell | SSO buttons + email/password fallback |
| G-02 | Switch tenant context without re-authenticating | seamless; preserves session (AUTH-06) | sub-level | top bar tenant switcher | dropdown/menu in header |
| G-03 | See the health of all environments and applications in my project at a glance | clear; ≤3 clicks to logs/health from dashboard (NFR-03) | top-level | project dashboard | summary cards + status badges |
| G-04 | Deploy a specific build to an environment quickly and safely | reversible (rollback INS-05); rolling-update with auto-rollback (INS-04); deployable in ≤3 clicks from project dashboard (NFR-03) | top-level | environment overview | build picker + confirm modal |
| G-05 | Diagnose a runtime issue using logs and metrics | log filters by time/keyword/severity (OBS-02); near-real-time tail (OBS-03); query results <5s for last 24h (NFR-31) | top-level | logs panel + metrics dashboard | log viewer with tail toggle; metric tiles |
| G-06 | Build, view, and cancel application builds | build status visible (BLD-05); real-time and post-hoc logs (BLD-04); clear failure output (BLD-07) | sub-level | application detail → builds tab | build history list + log streamer |
| G-07 | Manage application source linkage and build branch | edit branch (BLD-02); edit metadata (APP-06); validate Git repo at registration (APP-01) | sub-level | application settings | form with validation feedback |
| G-08 | Manage instance lifecycle (start/stop/restart, scale, resource profile, health check) | one-click start/stop/restart (INS-03); scale 1–10 (INS-07); preset resource profiles (INS-10, INS-11) | sub-level | instance detail | inline action buttons + side panel form |
| G-09 | Provision and manage databases for an environment | provisioning state visible (DB-03); deletion safety (DB-06) | sub-level | environment → databases tab | resource list + provision wizard |
| G-10 | Provision and manage object storage buckets for an environment | usage visible (OBJ-05); deletion safety (OBJ-02) | sub-level | environment → buckets tab | resource list + provision wizard |
| G-11 | Link/unlink databases and buckets to/from instances safely | same-environment-only (LNK-01); restart on link change (LNK-06); audited (LNK-08) | interaction-level | instance detail → linked resources panel | link/unlink action with restart-warning modal |
| G-12 | Manage environment- and instance-level configuration values and secrets | precedence shown (CFG-02); secret values write-only (SEC-04); restart on change (CFG-03) | sub-level | configuration tab | key/value table with secret toggle |
| G-13 | Expose an instance publicly via the API gateway | only via gateway (NET-02); generated public URL (NET-04); per-port path prefix routing | sub-level | instance → networking tab | toggle + path/port editor |
| G-14 | Trigger and restore from backups | manual on-demand backup (NFR-62); restore confirmation (NFR-64); audited (NFR-65) | sub-level | resource detail → backups tab | backup list + restore wizard |
| G-15 | Receive completion notifications for builds, deployments, and provisioning | retention last 50 / 30 days (NOT-03); covers builds, deployments, resource provisioning (NOT-02) | interaction-level | top bar notification tray | toast + bell tray |
| G-16 | Invite, deactivate, and reactivate tenant members | email invitation with delivery-failure surfacing (USR-02); deactivation preserves audit history (USR-05) | sub-level | tenant → users page | directory table + invite modal |
| G-17 | Manage user roles at tenant, project, and environment scope | granular four-level RBAC (RBAC-03); audited (RBAC-06); cannot remove last tenant admin (RBAC-07) | sub-level | role assignment matrix | matrix or per-user editor |
| G-18 | Create, edit, and delete projects within a tenant | name/description editable (PRJ-06); deletion blocked while resources exist (PRJ-07) | sub-level | tenant → projects page | list + create wizard |
| G-19 | Create, edit, and delete environments within a project | display name editable (ENV-08); deletion blocked while resources exist (ENV-09) | sub-level | project → environments page | list + create wizard |
| G-20 | Search and inspect the audit trail | filterable by user/project/action/resource/time/environment (AUD-04); immutable (AUD-05) | sub-level | tenant → audit page | filter form + paginated table |
| G-21 | Manage tenants at platform level (create, suspend, reactivate, delete) | requires url_id confirm to delete (PADM-06); platform admin cannot view tenant-internal data (PADM-04) | top-level | platform admin console | tenant list + side drawer detail |
| G-22 | Grant and revoke the platform administrator role | cannot remove the last platform admin (PADM-08); audited (PADM-09) | sub-level | platform admin → admins page | grantee picker + confirm modal |
| G-23 | View internal address and public URL of an instance | internal DNS surfaced (NET-01); generated URL surfaced (NET-04) | interaction-level | instance overview side panel | copyable address fields |
...
| Decision points | If email/password is disabled by the tenant, only SSO buttons are shown (AUTH-04). If user has a single tenant membership, route directly to that tenant; otherwise show tenant chooser. |
...
| Role-conditional behaviour | Platform Administrators may have no tenant membership; route them to the platform admin console instead of a tenant chooser. |
...
| Exception paths | Stop-instance failure does not block suspension; failed stops are recorded for retry. |
...
| Exception paths | Restart failure → instance enters failed state and surfaces in logs/metrics. |
...
| Exception paths | Restore failure → previous state preserved where possible; failure surfaced in notifications and audit. |
...
| Re-auth scope | Step-up re-auth for: tenant deletion, platform-admin role grant/revoke, secret rotation, restore-from-backup. | inferred |
| Account lockout policy | 5 failed attempts → 15-minute cooldown for email/password; SSO failures handled by IdP. | inferred |
| MFA requirement | Required for Platform Administrators; recommended for Tenant Administrators; otherwise per IdP policy. | inferred |
...
| p95 page TTI on dashboard screens | < 2s | inferred |
| Log tail end-to-end latency | < 3s | inferred |

#### 6.6.3 Availability

| Field | Value | Source |
| --- | --- | --- |
| Target uptime | 99.9% | inferred (HA design per NFR-20) |
| Maintenance window | Outside business hours, customer-configurable per installation | inferred |
...
- Data residency determined by the on-premise / hosted installation choice; the portal does not assume a specific jurisdiction.
- POPIA / GDPR alignment expected for hosted deployments; specific compliance regimes to be confirmed per installation.
- PCI-DSS scope: out of scope (the portal does not process payment card data).

#### 6.6.5 Accessibility

- WCAG 2.2 AA target.
- Keyboard-navigable enterprise console; visible focus states; sufficient contrast within the provided colour tokens.
- Screen-reader support for primary tasks (dashboards, logs, deploys).
...
**Enums:** HealthStatus = {`running`, `degraded`, `stopped`, `failed`}; ResourceProfile = {`small`, `medium`, `large`}. (`custom` excluded from v1.)
...
| read_at | Timestamp | no | — | NOT-03 |

**Domain concept:** Notification

**Relationships:** User 1—* Notification.

**Enums:** NotificationEvent = {`build`, `deployment`, `resource_provisioning`}; NotificationPhase = {`start`, `completion`}; NotificationStatus = {`succeeded`, `failed`, `cancelled`, `rolled_back`}.
...
| Container | A packaged, versioned, runnable instance of an application (per requirements §2). | "Container" is requirements terminology; the equivalent persistent entity is ContainerImage in the domain model. |
...
| Resource profile | Preset CPU/memory allocation: Small / Medium / Large (INS-10, INS-11). | Domain model lists a `custom` value; not in v1 inputs — excluded from v1. |
...
| Projects per tenant | Up to ~50 | inferred |
| Applications per project | Up to ~30 | inferred |
| Instances per environment | Up to ~30 | inferred |
| Replicas per instance | 1–10 (default 1) | stated (INS-07) |
| Builds per application per day | Up to ~20 | inferred |
| Concurrent users (portal-wide) | Up to ~500 | inferred (horizontal scaling per NFR-42) |
| Concurrent users (per tenant) | Up to ~50 | inferred |
