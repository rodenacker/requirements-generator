...
| Mode-specific invariants appendix | appended for prototype builds | omitted | yes — merger conditional |
...
| Deferred | Advisor-facing review and approval console |
...
| --- | --- | --- |
| Client-side state management | → §6.1 F-10 / → §6.1 F-12 / → §10 | in-memory session model acceptable at this volume |
| Offline cache / local persistence | → §6.1 F-11 / → §10 | |
| File upload / binary blob handling | → §6.1 F-03 / → §6.1 F-04 | binary blob storage tier required |
| Notification delivery surface | → §6.8 NT-01 / → §6.8 NT-02 | in-app channel only |
...
| Lifecycle states | Not Started → In Progress → Ready to Submit → Submitted |
...
| --- | --- | --- | --- |
| Not Started → In Progress | Client begins onboarding [SRC: C-037] | Valid email provided [SRC: C-038] | Session is created and stored locally for resumption [SRC: C-039]; wizard advances to Step 2 |
...
| Role-conditional behaviour | None — single self-service persona |
...
| Exception paths | {Email does not match the one used to start onboarding → identity check fails → re-enter the correct email} |
| Role-conditional behaviour | None — single self-service persona |
...
| Decision points | Which section the client chooses to edit |
| Exception paths | {Validation error on an edited field → inline corrective message → fix the field before saving} |
| Role-conditional behaviour | None — single self-service persona |
...
| Exception paths | {Submission fails → apologetic, actionable system message → retry submission} |
| Role-conditional behaviour | None — single self-service persona |
...
| F-12 | The client can navigate forward and backward through the wizard steps [SRC: C-010] | Forward and backward navigation between steps is available | inferred |
...
| --- | --- | --- | --- |
| OnboardingSession.email | required | Email is required [SRC: C-128] | "Please enter your email address." |
| OnboardingSession.email | format | Valid email format [SRC: C-129] | "Please enter a valid email address." |
| Applicant.dateOfBirth | business-rule-ref | Age must be 18 years or older → BR-01 [SRC: C-035] | "You must be 18 years or older to open an account." |
| Applicant.emailConfirmation | cross-field | Email confirmation matches Step 1 email → BR-03 [SRC: C-123] | "This email does not match the one you started with." |
| Applicant.mobileNumber | format | Mobile number valid format [SRC: C-130] | "Please enter a valid mobile number." |
| Applicant.nationality | required | Nationality must be selected [SRC: C-131] | "Please select your nationality." |
| IdentityDocument.file | enum | Supported file type (JPG, PNG, PDF) → BR-04 [SRC: C-132] | "Supported file types are JPG, PNG, and PDF." |
| IdentityDocument.file | range | Maximum 5 MB per file → BR-04 [SRC: C-133] | "Each file must be 5 MB or smaller." |
...
| EmploymentProfile.otherExplanation | length | If "Other" source of funds, minimum 20 characters → BR-05 [SRC: C-125] | "Please provide an explanation of at least 20 characters." |
| OnboardingSubmission.declarations | required | All required declarations accepted → BR-06 [SRC: C-126] | "Please accept all declarations to submit." |

### 6.4 UI feature needs

| ID | Feature need | Linked (G / story / BR) | Acceptance criteria |
| --- | --- | --- | --- |
| UI-01 | The client can move forward and backward through the steps [SRC: C-010] | → §4.1 G-01 | Both forward and backward navigation are available between steps |
...
| --- | --- | --- | --- |
| → §5 onboarding flow (upload) | error | An apologetic, actionable message is shown when a document cannot be uploaded [SRC: C-086] | The client retries the upload |
| → §6.1 F-14 (upload) | loading | An upload progress indicator is shown while a document uploads [SRC: C-139] | Wait for upload to complete |
...
| → §6.1 F-13 (any step) | error | Specific, corrective validation messages are shown for user mistakes [SRC: C-140] | The client corrects the highlighted field |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Prospective Client | C R U | C R U | C R U D | C R U D | C R U | C R U | C R | X | X | X | X |
...
| Idle warning lead-time | 60 seconds (T-1 min) | inferred |
...
| --- | --- | --- |
| Time to interactive (p95) | p95 ≤ 2.0s | inferred |
| Initial bundle size budget | ≤ 250KB gzipped | inferred |
...
> No client-facing reporting is described in the inputs; this onboarding flow produces a single submission for downstream advisor review rather than reports. No RPT rows are emitted.
...
| Metric | Value | Source |
| --- | --- | --- |
| Data volume | One onboarding session per client; on the order of 10²–10³ in-progress sessions retained at a time | inferred |
| Frequency | Not specified | inferred |
| Concurrency | ~10 concurrent clients onboarding at a time | inferred |
