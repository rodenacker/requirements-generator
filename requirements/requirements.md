...
**Domain:** financial transaction management **Target:** application **Created:** 2026-05-26 **Status:** final **Last finalised at:** 2026-05-26

> Inline `[SRC: C-NNN]` tags reference verbatim source quotes in `requirements/draft-claims.ndjson`. Downstream LLM consumers (review, design, code-gen) join the tag to the sidecar on demand.
...
**Domain:** financial transaction management

**Business goal:** Provide controlled file-based transaction ingestion with single-Approver review of each transaction to reduce manual processing errors and produce an auditable transaction-decision trail.
...
> §1.5 enumerates the capability scope of this frontend build: what is in scope, what is explicitly out of scope, and what is deferred to a later phase.
...
| Out | server-side file parsing internals, backend identity provider implementation |
| Deferred | bulk file setting configuration UI, scheduled / unattended file imports |
...
| Client-side search / filtering | → §6.1 F-21 / → §6.7 RPT-01 | in-memory index acceptable given ≤10⁴ records per file |
| File upload / binary blob handling | → §6.1 F-04 | binary blob storage tier required |
...
| Role | persistent | A named bundle of access permissions assigned to one or more Users. |
...
| Key invariants   | A File Log cannot transition to Completed while any contained Transaction is still being processed. |
...
| ------------------------------- | ----------------------------- | ---------------------------------------------- | ----------------------------------------------------------------------- |
| Uploaded → Processing | server begins extracting transactions from the uploaded file [SRC: C-030] | the file has been received by the backend [SRC: C-031] | the File Log row shows status Processing in the file log list |
| Processing → Completed | all transactions extracted and persisted | → §6.2 BR-01 | the File Log row shows status Completed and the file summary becomes available [SRC: C-032] |
| Processing → Failed | extraction encountered an unrecoverable error [SRC: C-033] | → §6.2 BR-02 | the File Log row shows status Failed and a retry-validation action becomes available [SRC: C-034] |
...
| --- | --- | --- | --- | --- | --- |
| G-01 | Successfully import transaction files for downstream approval [SRC: C-044] | uploads visibly succeed; record counts match expectation; processing status is observable | top-level | — | — |
| G-02 | Maintain control over which transactions are approved and which are rejected with reason [SRC: C-045] | each transaction has a deliberate decision; rejected transactions carry a captured reason; status changes are immediate | top-level | — | — |
...
| G-05 | Locate specific transactions and files quickly across the working set [SRC: C-006] | filter and search return results within a perceptibly short time; active filters are visible | interaction-level | — | — |
...
| Acceptance criteria                      | Given the Importer has applied one or more filter dimensions, when the filter set changes, then the visible result set updates and the active filter set is observable in the UI. |
...
| Acceptance criteria                      | Given the Approver has selected a transaction with Status = Imported, when they invoke the approve action and confirm, then the transaction's Status transitions to Approved and the approve/reject actions are no longer available on that row. |
...
| Context (frequency / expertise / stakes) | daily |
| Linked task flow (optional)              | → §5 Flow: Search and Filter                                                                  |
| Acceptance criteria                      | Given the Approver has applied one or more filter dimensions, when the filter set changes, then the visible result set updates and the active filter set is observable in the UI. |
...
| Exception paths            | {no transactions match the active filter → empty-results copy referencing the filter, with an action to clear filters → user clears or adjusts filters} |
...
| Exception paths            | {transaction is no longer in Imported state when confirm is invoked → inline message that the action is no longer applicable → user re-opens the transaction or applies a different filter} |
...
| Exception paths            | {note is empty → inline required-field error message → user enters a non-empty note}; {transaction is no longer in Imported state when submit is invoked → inline message that the action is no longer applicable → user re-opens the transaction or applies a different filter} |
...
| Exception paths            | {filter set yields zero rows → empty-results message and the export action is disabled → user clears or adjusts filters} |
...
| Exception paths            | {File Log has zero transactions extracted → empty-state copy with the File Log's status and any available retry action → user retries validation or cancels} |
...
| Exception paths            | {no results → empty-results copy with the active filters visible and a clear-all action → user clears or adjusts filters} |
...
| F-02 | The system signs the user out and invalidates the active session. [SRC: C-092] | Given the user is signed in, when they invoke logout, then the active session is invalidated and the user is returned to the login surface. | stated / → §6.10 |
...
| F-06 | The system retrieves per-File-Log process log entries for a specific LogId. [SRC: C-096] | Given the user opens process detail for a specific File Log, when the entries are requested, then the system returns the activity history for that File Log. | stated / → §6.10 |
| F-07 | The system supports downloading the parsed data file for a File Log by LogId. [SRC: C-097] | Given the user requests the parsed data file for a File Log, when the request is invoked, then the file content is returned as a binary stream. | stated / → §6.10 |
| F-08 | The system supports downloading the original uploaded file for a File Log by FileLogId. [SRC: C-098] | Given the user requests the original uploaded file, when the request is invoked, then the file content is returned as a binary stream. | stated / → §6.10 |
...
| F-17 | The system lists, creates, updates, and deletes Users (administrative scope). [SRC: C-110] | Given an administrator invokes user-management operations, when each operation is submitted, then the system reflects the resulting User set and the operating user's identity is recorded on each change. | stated / → §6.10 |
| F-18 | The system lists, creates, updates, and deletes Roles, including Role-to-Page assignment (administrative scope). [SRC: C-111] | Given an administrator invokes role-management operations, when each operation is submitted, then the system reflects the resulting Role set and Page assignments. | stated / → §6.10 |
...
> Field-level validation surfaced to the user as inline UI feedback (required-field markers, format hints, range/length errors). Synchronous rules (format, required, length) validate on blur; cross-field and asynchronous rules validate on submit; never validate on keystroke. Backend enforcement of business invariants belongs to §6.2 BR-NN and the sibling backend doc; this section captures the *visible* validation surface only.
...
| Transaction.Reference | format | Must follow the file's reference format (alphanumeric with hyphens). | "Reference format is not recognised." |
| Transaction.Amount | range | Amount must be a numeric value with at most two decimal places. | "Amount must be a numeric value." |
| Transaction.Currency | enum | Currency must be one of the configured ISO-4217 codes (e.g. ZAR). [SRC: C-117] | "Currency code not recognised." |

### 6.4 UI feature needs

> *What UI elements and behaviours the FE must provide.* Never layout, position, framework, component name, or visual design — layout and visual design are produced by a later UX design step. Phrase behaviourally ("user can filter by status", "save action is available"); do not phrase visually ("filter chips in the toolbar"). UI feature rows that follow project-wide deterministic conventions (validation timing, required-field marking, autofocus, empty-state copy, loading thresholds, pagination, sortable tables, form-length escalation, toast/banner placement, badge caps, status colours, icon-only labelling, table-to-card collapse) are noted inline.
...
| UI-03 | The user can drill into a File Log to view its contained transactions. [SRC: C-025] | → §5 Flow: File Log Overview | Selecting a File Log entry navigates the user to the transactions screen filtered to that file. |
| UI-04 | The user can view transactions as a sortable, filterable list showing Reference, Date, Account, Amount, Currency, and Status. [SRC: C-107] | → §4.2 Story: Approver Transaction Review / → §6.1 F-14 | All listed fields are sortable; sorting is single-field; active sort is observable and persists for the session. |
| UI-05 | The Approver can invoke approve or reject on a transaction when the status is Imported. [SRC: C-058] | → §4.2 Story: Approver Approve / → §6.1 F-15 / → §6.1 F-16 | Approve and reject are visible only when Status = Imported; both invoke a confirmation gate; reject requires a non-empty user note. |
| UI-06 | The user can apply filters across status, file, date range, amount range, and free-text search on Reference or Account. [SRC: C-054] | → §4.2 Story: Search and Filter / → §6.1 F-21 | Active filters are observable and removable individually; clearing all filters returns the unfiltered view. |
...
| UI-11 | The application chrome shows the authenticated user's name and offers a logout affordance. [SRC: C-094] | → §6.1 F-03 / → §6.1 F-02 | The chrome reflects the authenticated user; invoking logout invalidates the session and returns the user to the login surface. |
| UI-12 | The pagination affordance is always rendered on transaction and file-log lists with rows-per-page selector. | → §6.1 F-14 / → §6.1 F-05 | Rows-per-page offers 5, 10, 20, 50 with 20 as the default; navigation controls are visible even when only one page exists. |
| UI-13 | Required fields are marked with a leading asterisk and an explanatory legend. | → §6.3 | Required-field markers are present on the login form and the reject-with-note form. |
| UI-14 | Status badges use the standard intent-to-colour mapping with paired icon or text label. | → §6.1 F-14 / → §6.1 F-05 | Imported → blue/info; Approved → green/success; Rejected → red/error; never colour-only. |
| UI-15 | Icon-only per-transaction actions expose a hover-and-focus textual label and `aria-label` matching the action name; primary destructive actions are not icon-only. | → §6.1 F-15 / → §6.1 F-16 | Approve and reject per-transaction controls expose a textual label on hover and focus via `aria-label`; destructive actions are not represented as icon-only controls. |
| UI-16 | Empty states name the entity ("No files yet", "No transactions yet") and offer the appropriate creation CTA when permitted. | → §5 Flow: File Log Overview / → §5 Flow: Transaction Review | Each empty state surfaces a specific copy; zero-results-from-filter shows the active filter chips and a clear-all action instead. |
...
| -------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------- | ------------------------------------- |
| → UI-02                  | empty  | Empty-state copy reads "No files uploaded yet" and presents the upload CTA to Importers only. | Importer uploads a file; Approver waits or contacts an Importer. |
| → UI-04                  | empty | Empty-state copy reads "No transactions yet" with no action. | User waits for an Importer to upload a file. |
| → UI-06                  | empty | Empty-results copy references the active filter and offers a clear-all action; no creation CTA. | User clears or adjusts filters. |
| → UI-04                  | loading | A skeleton matching the list is shown for the 300 ms–3 s range; a still-loading message is added beyond 3 s. | User waits; manual retry not required. |
...
| → UI-05                  | permission-denied | Approve and reject actions are hidden from non-Approvers; the transaction entry reads cleanly without disabled controls. | None — the action is not offered. |
| → UI-01                  | permission-denied | Upload affordance and navigation entry are hidden from non-Importers. | None — the action is not offered. |
...
| → UI-07                  | error | Export failure surfaces an actionable error message; the action remains invokable. | User retries the export. |
| → §5 Flow: Authentication | error | Login failure surfaces a generic incorrect-credentials message that does not reveal which field was incorrect. [SRC: C-091] | User re-enters credentials. |
...
> Frontend NFRs only. Backend availability / throughput / persistence concerns live in the sibling backend requirements doc.

#### 6.6.1 Session UX

| Field                    | Value                                                                             | Source            |
| ------------------------ | --------------------------------------------------------------------------------- | ----------------- |
| Idle session timeout     | 15 minutes | inferred (financial domain) |
| Absolute session timeout | 8 hours | inferred (financial domain) |
| Idle warning lead-time   | 60 seconds before idle logout | inferred (financial domain) |
| Re-auth scope            | step-up authentication required for approve and reject actions | inferred (financial domain) |
| Account lockout messaging | No account-lockout messaging in the initial scope; failed login attempts surface the standard generic credentials-incorrect message only. | inferred |
...
| ----------------------------------------------------- | --------- | ----------------- |
| Time to interactive (p95)                             | p95 ≤ 2.0 s | inferred |
| Initial bundle size budget                            | ≤ 300 KB gzipped | inferred |
| Render budget for largest list/table                  | p95 ≤ 500 ms for a page of 50 transaction rows | inferred |
| Time to meaningful content                            | p95 ≤ 1.5 s on the file log overview | inferred |
...
- PII fields (account number, user name) are not masked in the visible UI; access is controlled by RBAC rather than masking.
- No regional UI variants are required in the initial scope.

#### 6.6.5 Accessibility

- WCAG 2.2 AA is the accessibility target across all screens.
- Keyboard-only navigation must reach every interactive element including approve, reject, export, and filter controls.
- Status conveyance never relies on colour alone — every status badge pairs colour with an icon or text label.

### 6.7 Reporting feature needs

> Each entry captures *what reporting must exist*, never *how it is visualised*. Chart type, layout, and visualisation choice are determined by the later UX step.
...
| --- | --- | --- | --- | --- |
| NT-01 | A File Log has finished processing (Completed or Failed). | Importer | in-app | → §6.2 BR-01 / → §6.2 BR-02 |
| NT-02 | A File Log is now ready for Approver review (all transactions Imported). | Approver | in-app | → §6.2 BR-01 |
...
| ------------- | -------------- | ------------------------- | ------------------------- |
| Transaction    | Status, UserNote, LastChangedUser, LastChangedDate [SRC: C-122]     | most recent change per Transaction visible in the detail view; full history retention is the backend doc's concern | Approver |
| FileLog | LastChangedUser, LastChangedDate, CurrentStatus, LastExecutedActivityName [SRC: C-123] | most recent change per File Log visible in the detail view | Importer, Approver |
...
| UserGetList / UserCreate / UserGetById / UserUpdate / UserDelete (/v1/users) | → backend doc operations for User CRUD [SRC: C-110] | Administrative User management; each mutating operation also carries the operating user's identity in a header. |
| RoleGetList / RoleCreate / RoleUpdate / RoleDelete (/v1/roles) | → backend doc operations for Role CRUD [SRC: C-111] | Administrative Role management. |
| PageGetList (GET /v1/pages) | → backend doc operation PageGetList [SRC: C-112] | Returns the catalogue of assignable Pages. [SRC: C-112] |
| FileSetting / FileSource / FileType / FileLocationType / FileLocation / ProcessDefinition / BulkFileSettingDatabase / BulkFileSetting list and update operations | → backend doc operations for file-settings management | Administrative configuration of file-handling parameters. |

---

## 7. Data shapes consumed by the FE

> Shape of data the FE reads and writes — the shape of payloads exchanged with the backend (authoritative shape lives in the sibling backend requirements doc). Persistence design — indexes, FK constraints, storage layout — is the backend doc's concern, not this section's.
...
**Source:** backend-contract
**Enums:** Name ∈ {Importer, Approver, …} (extensible via Role administration)
...
| Login endpoint | The user-authentication endpoint. | The project brief names this `POST /v1/users/login` [SRC: C-018]; the authentication API contract names this `POST /v1/auth/login` [SRC: C-090]. The contract is the authoritative source. |
...
| Metric      | Value                 | Source                |
| ----------- | --------------------- | --------------------- |
| Data volume | 10²–10⁴ transactions per File Log; 10³–10⁵ Transactions in the active working set per Approver session | inferred from sample CSV (20 rows) and typical batch import scale |
| Frequency   | 1–10 file uploads per Importer per day; 10²–10³ approve/reject actions per Approver per day | inferred |
| Concurrency | 1–10 concurrent Importer + Approver sessions | inferred |
