...
**Domain:** Wealth management / financial services [SRC: C-003] **Target:** prototype **Created:** 2026-05-30 **Status:** draft **Last finalised at:** —

> **Authoring guardrails.** Cells obey `GR-20` (no stack specifics — capability categories only) and `GR-21` (no UI layout in §6.4/§6.7/§6.8/§6.9). Inferred content carries one of `[AI-SUGGESTED]`, `[STANDARD-RULE: GR-NN]`, or `[OUT-OF-SCOPE: domain-default]`. Input-grounded cells carry `[SRC: C-NNN]`.
...
| Deferred | Joint-account and multi-applicant onboarding; advisor-side review and approval surfaces [AI-SUGGESTED: AI-001 | non-blocking] |
...
| --- | --- | --- |
| Client-side state management | → §6.1 F-01 / F-05 / F-31 | [AI-SUGGESTED: AI-002 | non-blocking] |
| Offline cache / local persistence | → §6.1 F-05 / F-30 / F-31 | [AI-SUGGESTED: AI-003 | non-blocking] |
| File upload / binary blob handling | → §6.1 F-11 / F-12 / F-13 / F-29 | binary blob storage tier required [AI-SUGGESTED: AI-004 | non-blocking] |
| Notification delivery surface | → §6.8 NT-01 / NT-02 / NT-03 | in-app channel only [AI-SUGGESTED: AI-005 | non-blocking] |
...
| --- | --- | --- | --- |
| In Progress → Submitted | The client confirms and submits from the review step [SRC: C-031] | All six steps complete with required uploads and declarations provided → §6.2 BR-01 [SRC: C-032] | The success screen replaces the flow and a confirmation number is shown [AI-SUGGESTED: AI-006 | non-blocking] |
| Submitted → Confirmed | The mock submission endpoint returns success with a generated confirmation number [SRC: C-033] | → §6.2 BR-04 | The confirmation number, follow-up message, and contact email are shown and local storage is cleared [AI-SUGGESTED: AI-007 | non-blocking] |
...
| Frequency of use | One-time onboarding, potentially completed across multiple sessions via save-and-resume [OUT-OF-SCOPE: domain-default] |
...
| --- | --- | --- | --- |
| F-01 | Provide a six-step onboarding flow [SRC: C-057] | Given a started session, when the client advances, then they move through exactly six ordered steps to a submission state [AI-SUGGESTED: AI-008 | non-blocking] | stated |
...
| UI-15 | Field validation runs on blur for format and required rules and on submit for cross-field rules [STANDARD-RULE: GR-05] | → §6.1 F-04 | No validation fires on keystroke |
| UI-16 | Required fields are marked and explained with a single legend [STANDARD-RULE: GR-06] | → §6.1 F-09 | Required markers are consistent across forms |
| UI-17 | The first editable field receives focus when a step opens [STANDARD-RULE: GR-07] | → §6.1 F-09 | Keyboard users can type immediately |
| UI-18 | A loading affordance appears for the upload simulation and clears on completion [STANDARD-RULE: GR-10] | → §6.1 F-14 | No flicker for sub-300ms operations; a skeleton/progress affordance for longer ones |
| UI-19 | Transient confirmations use auto-dismissing messages; state the user must acknowledge persists [STANDARD-RULE: GR-14] | → §6.1 F-04 | Persistent state (errors, offline) is acknowledged, not auto-dismissed |
| UI-20 | The risk-category indicator pairs colour with a text label, mapped by intent [STANDARD-RULE: GR-16] | → §6.1 F-21 | Colour is never the sole signal of category |
| UI-21 | Icon-only controls expose an accessible name on hover and focus [STANDARD-RULE: GR-17] | → §6.1 F-14 | Each icon-only control has a matching accessible label |
...
| → §5 Save and resume / UI-05 | empty | When no saved progress exists for a returning client, the flow starts fresh from the first step [AI-SUGGESTED: AI-009 | non-blocking] | The client begins onboarding |
| → §6.1 F-25 / UI-12 | error | A submission failure shows an apologetic, actionable message and keeps entered data intact [STANDARD-RULE: GR-14] | The client retries submission |
| → §6.1 F-19 / UI-11 | partial | When not all five risk questions are answered, scoring is unavailable and the unanswered questions are indicated [STANDARD-RULE: GR-06] | The client answers the remaining questions |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Prospective Client | C R U | C R U X | C R U | C R U | C R U | C R U | C R U | C R U | R | R | X | X | X | X |

> Discarding an in-progress application and restarting (Delete access) is not specified by the brief [AI-SUGGESTED: AI-010 | blocking].
...
| --- | --- | --- |
| Time to interactive (p95) | p95 ≤ 2.5 s on a desktop viewport [AI-SUGGESTED: AI-011 | non-blocking] | inferred |
| Initial bundle size budget | ≤ 300 KB gzipped [AI-SUGGESTED: AI-012 | non-blocking] | inferred |
...
- Target conformance: WCAG 2.2 AA [AI-SUGGESTED: AI-013 | non-blocking].
...
| --- | --- | --- |
| KYC (Know Your Customer) | Regulatory practice of verifying a client's identity and suitability during onboarding [OUT-OF-SCOPE: domain-default] | — |
| AML (Anti-Money Laundering) | Regulatory controls that source-of-funds and identity collection support [OUT-OF-SCOPE: domain-default] | — |
| Suitability assessment | Evaluation of whether an investment approach fits the client, informed by the risk profile [OUT-OF-SCOPE: domain-default] | — |
...
| Metric | Value | Source |
| --- | --- | --- |
| Data volume | One onboarding application per client; a handful of uploaded documents per application [AI-SUGGESTED: AI-014 | non-blocking] | inferred |
| Frequency | Low — onboarding is a one-time event per client, occasionally resumed across sessions [AI-SUGGESTED: AI-015 | non-blocking] | inferred |
| Concurrency | A single client per session; no shared or concurrent editing [AI-SUGGESTED: AI-016 | non-blocking] | inferred |
