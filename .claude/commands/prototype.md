...
6. Scaffold the shared app once (`framework/agents/prototype-app-scaffolder.md`) if not already scaffolded — this is where the brand is captured: a `design-system/design-system-light.html` or `-dark.html` (surfaced as the Brand source at input selection) themes the app, and if none exists you're recommended to run `/design-system` first.
