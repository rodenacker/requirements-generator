...
1. Artefact picker — printed numbered list of `review-inputs/*/*.html` + `review-requirements/*/*.html`, **split into two clearly-headed groups (Input reviews / Requirement reviews) under one continuous number sequence** (friendly exit when none exist; `0` = cancel), then the methodology-map gate against `framework/assets/resolve-review/methodology-map.md` (review-requirements artefacts key as root-qualified `review-requirements/<METHOD>`).
...
Honour the stale-draft gate and the drafter's handback gate defined in the orchestrator. Do not perform any task that is not listed in the orchestrator. The pipeline is stand-alone — it writes only `resolve-review/resolutions-draft.md` (transient staging), the single new `input/<stem>-<date>.md` (the additive-`input/` write exception, additive only), and — Step 9b opt-in only — the bounded Amendments section in `requirements/requirements.md` (the Amendments-section write exception, performed by `framework/skills/apply-amendments-section.md`; both exceptions are shared with `/amend-requirements`). No progress file, no timing events, no input-handler invocation; the new input file is picked up by the next source-manifest create/refresh.
