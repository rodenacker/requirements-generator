...
The agent's only outputs are `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` and the inline summary it surfaces to the consultant.
...
- Ensure the output directory exists: `Bash mkdir -p reviews/TEN-UX-QUESTIONS`.
- `Write reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` with the in-memory composed markdown.
- Invoke `framework/skills/verify-artifact-write.md` with `path = reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md`, `expected_sha256 = <Step-7 sha>`, `expected_min_bytes = 1024` (tighter than the default `1` — a minimum legal render with 10 question blocks, a triage table, and a diagnostics block is comfortably above 1 KB).
- On `pass`: advance to Step 9.
- On `RF-04 trigger`: halt per `framework/shared/refusal-registry.md > RF-04 artifact_write_unverified`. Emit the single line *"Aborting to protect your work — write verification failed for `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` after one retry."* and fail the handback. The orchestrator does not declare done.
...
> *"Wrote `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` — 10 UX questions selected from `{{CANDIDATE_POOL_SIZE}}` candidates. Priority: `{{BLOCKING_COUNT}}` blocking · `{{MAJOR_COUNT}}` major · `{{MINOR_COUNT}}` minor. Category coverage: `{{N}}` of 8. Quality gates: `{{n_gates_passed}}/8` pass. Ready, or want changes?"*
...
- **Restart** — re-enter Step 3 from a clean state. Generate a fresh candidate pool; re-filter; re-score; re-select. The previously-written `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` is left in place; the next Step 8 will overwrite it.
...
- `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` — the populated artefact. Always written to the same path; overwritten on each run (the orchestrator's prior-artefact gate has already taken the consultant's overwrite/keep/cancel choice before the agent is invoked).

## Tools

- `Read` — read the character file, the reference asset, the template scaffold, the merged requirements document, and (at Step 4 only) the three shared-policy filter sources (`framework/shared/general-rules.md`, `framework/shared/prototype-invariants.md`, `framework/shared/prototype-scope.md`). **Read is not authorised against any path under `requirements/` other than `requirements/requirements.md`, against any path under `analyses/`, against any path under `design-system/`, against any path under `framework/state/`, or against any other path under `framework/shared/`.** The stand-alone constraint is enforced by tool-list scope.
- `Write` — write `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md`.
...
- `Bash` — `mkdir -p reviews/TEN-UX-QUESTIONS` (Step 8 setup). No other Bash usage.
...
- `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` exists and `verify-artifact-write` returned `pass`.
...
- `reviews/TEN-UX-QUESTIONS/ten-ux-questions-review.md` exists, has been verified, and contains exactly 10 UX questions selected from a candidate pool of ≤ 50.
...
- Do not read `analyses/`, `design-system/`, or `framework/state/` for any purpose. Derivative artefacts and pipeline state are not 10-UX-Questions inputs.
