...
Per `framework/assets/reviews/template-ten-ux-questions.html`:
...
- Build the substitution map for the placeholders documented in the template's header comment:
    ...
    - `{{TRIAGE_BLOCK}}` — pre-rendered HTML `<tr>` rows (only the `<tbody>` rows; the `<thead>` is in the scaffold) per the TRIAGE BLOCK SCHEMA. Rows are in Rank order (UXQ-01 first). For each question, fill the Rank / ID (linking to `#{UXQ-NN}`) / Priority (`.chip.priority-{priority}`) / Category / Anchor / Question (first line) cells, and carry the `priority-{priority}` class on the `<tr>`. Truncate question text at first sentence or 100 chars, whichever is shorter; append `…` if truncated. HTML-escape cell content (no markdown pipe escaping — HTML table).
    ...
    - `{{DIAGNOSTICS_BLOCK}}` — pre-rendered diagnostics per the DIAGNOSTICS SCHEMA: a single `<details class="diagnostics-toggle" open>` wrapping the candidate-pool stats table, category-coverage table, 8-gate quality-gate table (PASS/FAIL `.chip`), override log.
...
- Do not edit the HTML scaffold in `framework/assets/reviews/template-ten-ux-questions.html`. Only the documented `{{placeholders}}` are substituted; the inline `<style>` block, section ordering, IDs, the TOC list, table column headers, and the diagnostics layout are fixed.
