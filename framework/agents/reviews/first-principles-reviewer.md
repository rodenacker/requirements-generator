...
- Build the substitution map for the placeholders documented in the template's header comment:
    - `{{PLAIN_SUMMARY}}` — 2–5 sentences; a faithful condensation of the run's findings: what this review is (the First Principles defensibility audit — checking whether each §4–§7 artefact chains back to a stated business reality), what it found (state the verdict `{{VERDICT}}` plainly and unsoftened — if the verdict is `BLOCKED`, say so; name the dominant trigger, e.g., *"N subjects scored 0/6"* or *"N orphan goals"* or *"N blocking cross-subject findings"*), and what the consultant should do next (open the file in a browser, start from the Top 10). No finding or count not present in the sections below. **Preserve severity verbatim** — never soften a `BLOCKED` verdict or a blocking finding into reassurance. Gloss review jargon at first use (e.g. *"defensibility score (count of questions answered with verbatim evidence, 0–6)"*, *"orphan (an artefact that lacks its required counterpart)"*, *"cross-subject finding (a relation the subjects collectively cannot satisfy)"*); **do not gloss client domain terms**. HTML-escaped.
