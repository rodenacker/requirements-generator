...
3. **Build the cumulative state.** Construct:
    ...
    - The severity tally (Blocker / Major / Minor counts) and disposition tally (Patch / Defer / Reject counts). **These are provisional** — Step 3s recalibrates ratings and recomputes both tallies before the verdict is derived.
...
After Step 3b completes, the in-memory state is identical in shape to what the sequential pipeline produced after Step 9b — except that the severity/disposition tallies are provisional until Step 3s.

### Step 3s — Scope recalibration (purpose-aware rating)
...
3. **Annotate + recompute.** Write each finding's returned `scope_class` and any adjusted `severity` / `disposition` back into the in-memory finding list. **Recompute** the severity tally and disposition tally from the recalibrated values — these supersede the provisional Step-3b tallies and drive the Step-11 verdict. Keep the `recalibration_log` in memory for the Step-11 diagnostics block.

4. **Invariants.** No finding is added, removed, or renumbered; no `evidence` / `problem` / `recommendation` text is edited; only `severity`, `disposition`, and the new `scope_class` metadata may change. `ADV-NN` IDs from Step 3b are final. The strict-BMAD re-run log and the Variant-A/B map are unchanged. `scope_class` is metadata, not a 9th schema field — quality gate 1 is unaffected.

The recalibration is annotation + re-rating only; the consultant can override any adjustment in the Step-13 Revise loop, where the recalibration log makes every change visible.

### Step 3c — Consolidate (cluster findings sharing a root cause)

After deterministic ID assignment in Step 3b and scope recalibration in Step 3s, the reviewer runs one consolidation pass over the merged finding list to identify **clusters** — groups of ≥2 findings that share a root cause. The pass is a navigation aid for the consultant, not a methodology change: no finding is dropped, rewritten, or merged; every `ADV-NN` retains its full Severity / Disposition / Location / Evidence / Problem / Recommendation, and the per-dimension sections render unchanged at Step 11. The triage selection below keys on the **recalibrated** `Reject` / `Blocker` membership (post-Step-3s), so a `backend-only` finding never appears in triage as a Blocker/Reject.
...
**Triage selection** (also kept in memory for Step 11). Compute the "Top issues to address first" list per the TRIAGE BLOCK SCHEMA in `framework/assets/reviews/template-adversarial.html`:
...
Per `framework/assets/reviews/template-adversarial.html`:
...
- Build the substitution map for the placeholders documented in the template's header comment:
    ...
    - `{{VERDICT}}` — derived per the reference's disposition-to-verdict mapping. This value is also the `.verdict-{{VERDICT}}` class suffix on the banner, so it must be exactly one of `BLOCKED | NEEDS-REVISION | ACCEPTED-WITH-FIXES`.
    - `{{TRIAGE_BLOCK}}` — pre-rendered "Top issues to address first" `<table class="triage-table">` per the TRIAGE BLOCK SCHEMA. Sourced from the triage selection computed in Step 3c. If the triage selection is empty (zero findings run-wide), substitute the single line `<p>No findings — strict-BMAD justification blocks below cover all eight dimensions.</p>`.
    - `{{CLUSTERS_BLOCK}}` — pre-rendered cluster summary `<table class="clusters-table">` per the CLUSTERS BLOCK SCHEMA. Sourced from the cluster metadata computed in Step 3c. If Step 3c produced zero clusters, substitute the single line `<p>No clusters — every finding stands on its own root cause.</p>`.
    - `{{FINDINGS_TABLE}}` — pre-rendered HTML `<tr>` rows (only the `<tbody>` rows; the `<thead>` is in the scaffold) per the FINDINGS TABLE SCHEMA in the template header. Rows are sorted by (Severity descending: Blocker → Major → Minor) then (Dimension ascending: 1..8) then (within bucket: worker emission order = ADV-NN ascending). The `Cluster` column contains the finding's `CL-NN` from Step 3c, or is empty for singletons. Each row carries the `severity-{Sev} disposition-{Disp}` classes and severity/disposition `.chip` spans; the ID cell links to the finding's `id="{ADV-NN}"` anchor in its dimension block. ADV-NN IDs are **not** renumbered by the sort.
    - `{{DIMENSION_1_BLOCK}}` … `{{DIMENSION_8_BLOCK}}` — pre-rendered dimension sections per the DIMENSION BLOCK SCHEMA. Each block is either Variant A (`<div class="findings-list">` of `<article class="finding" id="{ADV-NN}">` cards) or Variant B (`<div class="justification">`) — never both, never neither. Per-dimension findings retain their original within-dimension emission order (the severity-driven sort at Step 11 applies **only** to the Findings Table). Each finding card's header carries a `scope_class` chip (`<span class="chip scope-{scope_class}">{scope_class}</span>`) alongside its severity and disposition chips, sourced from the Step-3s annotation.
    - `{{DIAGNOSTICS_BLOCK}}` — pre-rendered diagnostics per the DIAGNOSTICS SCHEMA: a single `<details class="diagnostics-toggle" open>` wrapping the quality-gate table (11 gates with PASS/FAIL `.chip` + flagged items), coverage map (one row per dimension), strict-BMAD re-run log (one line per dimension that triggered the rule, or "No dimensions triggered the strict-BMAD re-run rule."), the **Scope recalibration log** (the Step-3s detected target + PI-block signal on one line, then one row per recalibrated finding: `ADV-NN`, `scope_class`, `original-severity/disposition → adjusted-severity/disposition`, foreclosing authority — or "No findings required scope recalibration." when the log is empty), and the override log (gates and flags if Override invoked, or "All quality gates passed; no override invoked.").
- **HTML-escape every substituted value** before injection — the five characters `&`, `<`, `>`, `"`, `'` become `&amp;`, `&lt;`, `&gt;`, `&quot;`, `&#39;`. This applies to every consultant-visible string: titles, locations, problems, recommendations, and the verbatim Evidence quotes (escape the quote, then place it inside `<blockquote class="evidence"><pre>…</pre></blockquote>` — the `<pre>` preserves the quote's line breaks verbatim). There is no markdown pipe escaping — the tables are HTML, not markdown.
- Compose the full HTML in memory. Compute SHA-256 of the in-memory bytes.

The template scaffold itself is **not edited** — section ordering, IDs, the `<style>` block, ARIA labels, table column headers, and the TOC list are fixed. Only the documented `{{placeholders}}` are substituted, and every substitution is a text value or a pre-rendered HTML fragment built per the schemas in the template header. No `<script>` tag, no external stylesheet, no CDN reference is ever introduced.
...
- **Revise** — accept the consultant's revision instructions in their next message. Apply the changes. This is the BMAD "human filter" stage where false positives are removed and dispositions tuned. Whenever a revision changes the finding set, its IDs, severities, or dispositions, **Step 3c is re-run** in full so that cluster membership and the triage selection reflect the post-revision state.
    ...
    **Scope recalibration on Revise.** Step 3s is **not** blanket-re-run over the existing finding set on Revise — doing so would fight a deliberate consultant override (e.g. the consultant restoring a `backend-only` finding to `Blocker` because they judge it genuinely blocking). A consultant severity/disposition edit is an explicit override that **sticks**; record it in the Scope recalibration log as `consultant-overridden` for that `ADV-NN`. Step 3s is re-run **only** over findings that are *newly introduced* during Revise (the "Strike all findings on a dimension" → re-dispatch-worker branch) or on a full **Restart**. Existing findings keep their Step-3s `scope_class` and any consultant override.
...
- `framework/skills/recalibrate-scope-severity.md` — the purpose-aware scope-recalibration procedure. Read once in Step 3s; classifies each finding and caps `backend-only` ratings against the detected build target.
...
- Step 3s ran: a build target (`prototype` | `application`) was detected from the PI-block presence/absence and recorded in the Scope recalibration log.
- Every finding carries a `scope_class` (`fe-relevant | fe-facing-contract | backend-only`).
- Every `backend-only` finding satisfies the caps: severity ≤ target cap (`Minor` under prototype, `Major` under application) — never `Blocker` — and disposition is never `Reject`. (Exception: a finding the consultant explicitly overrode in the Revise loop, which is marked `consultant-overridden` in the recalibration log.)
...
- Step 3s ran: every finding carries a `scope_class`, every `backend-only` finding is within its target cap (never `Blocker`/`Reject`, except consultant overrides), and the Scope recalibration log records the detected target plus every recalibrated finding.
...
- Do not edit the HTML scaffold in `framework/assets/reviews/template-adversarial.html`. Only the documented `{{placeholders}}` are substituted; the inline `<style>` block, section ordering, IDs, the TOC list, table column headers, and the diagnostics layout are fixed.
...
- Do not inject unescaped finding text into the HTML. Every substituted value (titles, locations, problems, recommendations, verbatim Evidence quotes) is HTML-escaped (`&` `<` `>` `"` `'`) before substitution; a raw `<` from a requirement quote would otherwise corrupt the markup.
