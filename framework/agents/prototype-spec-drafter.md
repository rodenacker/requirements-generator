...
- `prototype_identity` — passed by the orchestrator from Step B: `{ name, name_slug, scope_slug, posture, dimension_positions, device_targets, primary_persona, purpose_prose, wireframe_basis }`.
...
- `framework/assets/prototypes/ux-baseline-checklist.md` — the usability half of the §9 floor, **including the C1–C10 projection of the component minimum-feature contracts** (canonical owner `framework/assets/design-system-standards.md §1`) that §9's contracts + exceptions tables are populated from.
- `framework/assets/prototypes/visual-craft-standard.md` — the visual/tactile half of the §9 floor, and the canonical source for §4b's named viewports and per-target obligations (reference them; never restate them in the spec).
...
2. *(substep `populate-spec`)* Populate the template top-to-bottom in one pass — no `{{placeholders}}`, no blanks. Apply markers per **Marker scheme**:
    ...
    - **§4b Device & viewport targets** — copy the consultant's Step-B(5) answer verbatim into the table **and** into the front-matter `device_targets` object; the two must agree exactly, since the front-matter is what the generator reads mechanically. Mark the choice `[SRC: consultant]`. **Never infer targets from the posture, the surface mix, or the persona** — this is a consultant decision, not a derivable one, and a posture-derived guess would silently override an explicit answer. The one judgement here is the "Why" line (tie it to where the §3 persona actually works).
    - **§9 Accessibility, UX & visual-craft checklist** — the spec-relevant subset of **both** floors: `ux-baseline-checklist.md` (does the surface work) and `visual-craft-standard.md` (does it look and feel designed), with the posture's emphasized items called out and the §4b targets reflected in the responsive line. Do not drop the craft half: a spec that lists only the usability floor is the shape that produced prototypes passing every gate while rendering as unstyled defaults.
      Also populate §9's **Component minimum-feature contracts** table (canonical `design-system-standards.md §1`, projected as C1–C10 in `ux-baseline-checklist.md`): one row per surface, naming **only the families that surface's §7 inventory row actually composes** — §1 is a minimum-feature contract, not a required roster, so a surface with no table gets no table row. Reference the C-items; never restate the §1 definitions here. Every surface whose §7 row names a collection-table organism carries C1–C5, and its §7 row **must** also name `Pagination` (`shared-component-conventions.md §7.2` makes it required, not optional) — `step-07` fails the build on a mismatch between this table and the code.
      Leave the **Exceptions** table empty unless a surface genuinely composes a `ui/table` that is *not* a §1 data table (a fixed key/value summary, a 3-row drawer panel, a wizard review step). Then emit one row per waived item **with a reason** — the reason is the whole mechanism: it downgrades that one check to a warning and is echoed to the consultant at the Step-G accept. A row with no reason is a FAIL. Never use this table to record a miss that could have been fixed.
...
- Do not introduce visual/brand decisions (a palette, a type scale, a radius, a per-prototype visual character) — the brand is fixed and uniform (D1); the spec governs layout + workflow only. **This is not a licence to omit §9's visual-craft half or §4b.** Naming the *floor* every prototype must meet (press response, hover shift, elevation discipline, type hierarchy, responsive contract) is not choosing a brand — those bind to tokens the design system already produced. Selecting a *different* brand per prototype is what is forbidden.
- **Do not infer `device_targets`.** It comes from the consultant at Step B(5) and nowhere else; a posture-derived or persona-derived guess overrides an explicit answer with a plausible one.
