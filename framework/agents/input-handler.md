...
4. **Conditional preflight.** Two independent dependency probes; run each only when its tier is present in the classified rows. `Native-multimodal` (raster) rows need **no** preflight — Claude's vision is native.
    ...
    **4b. vector renderer (`Vector-renderable`).** If at least one classified row has `tier: "Vector-renderable"`, call `framework/skills/preflight-cli.md` with the renderer binary name(s) per `framework/shared/setup-instructions/visual-render.md` and `advice_path: "framework/shared/setup-instructions/visual-render.md"`. On `RF-01 trigger` (the generalized `dependency_missing` predicate, CLI-binary variant), surface it per `framework/shared/refusal-registry.md > RF-01` and branch:
    - `continue-skip` — for every row with `tier: "Vector-renderable"`, demote to `tier: "Unsupported"` and set `conversions_applied: "skipped — RF-01"`. These rows are skipped within step 5. Continue.
...
- `framework/skills/preflight-cli.md` — CLI-binary availability probe (step 4b; only when a `Vector-renderable` row exists).
