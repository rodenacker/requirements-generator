...
- `index.html`'s §3 grid has one `<section class="wf-meta-col">` per `successful_variants[i]`; each contains persona binding, design philosophy, strengths, weaknesses, tradeoffs, use_when, and **states-per-surface** (keyed by `LS-NN`) sourced **verbatim** from the corresponding `variant-position.json` + `manifest.json` (no paraphrasing, no truncation).
