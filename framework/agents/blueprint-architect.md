...
5. `step-05-compose-variants.md` — Only when `variants_output_path != null`. On `mode = "create"` and `mode = "regenerate-variants"`: surface a sequence of prompts to capture (a) which dimensions the consultant wants to diverge on (filtered by Section 2 of `tradeoff-dimensions-registry.md`), and (b) optionally an override of the cardinality (default 2; hard cap 3). Then compose N variant configurations — for each, choose a distinct position on each chosen dimension, bind to a persona from `personas_available`, validate per Section 4 (incoherent pairs) and Section 5 (persona-position compatibility); reject configurations that fail. On `mode = "add-variant"`: surface a single prompt for the additional variant's dimension positions + persona binding; merge into existing variants subject to the cap; do not regenerate existing variants.
...
- `AskUserQuestion` — surface the step-5 dimension-divergence + cardinality + per-variant persona-binding prompts; surface the step-7 conditional gate (accept / revise / cancel) on flag.
