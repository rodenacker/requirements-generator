...
- Feedback-pattern copy (toast/banner messages, validation error text, empty-state explanations, confirmation bodies) is consistent with the blueprint's Application character when one is recorded (tone only — no entity/field/label renames, no structural change).
