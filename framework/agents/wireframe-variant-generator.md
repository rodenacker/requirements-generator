...
- `wireframes.html` — variant landing page listing screens + a state matrix + cross-variant nav.
- `manifest.json` — per-screen pattern bindings + states rendered (consumed by the comparator for drift detection).
- `variant-position.json` — self-declared dimension positions + persona binding + strengths/weaknesses/use-when (consumed by the comparator for the trade-off matrix; **immutable mirror** of the architect's `variants.json` entry, augmented with self-authored strengths/weaknesses/use-when).
...
4. `step-04-compose-screens.md` — Loop over every `S-NN` row in the blueprint's inventory. For each: pick patterns per `pattern-bindings.md` + `tradeoff-dimensions-registry.md > Section 3`; selectively read those pattern catalogue entries; compose the per-screen HTML by token-substituting `template-screen.html`; embed `data-src` attributes; record per-screen pattern bindings + states into the in-memory manifest accumulator; write the screen file; verify.
5. `step-05-write-landing-and-sidecars.md` — Write `wireframes.html` from `template-variant-landing.html`; write `manifest.json` from the accumulator; write `variant-position.json` (the immutable mirror of own variants.json entry + self-authored strengths/weaknesses/use-when). Verify each.
6. `step-06-self-validate-and-handback.md` — Run the structural self-validation checks; on pass, hand back `ok` to the orchestrator; on fail, hand back a structured `failed` payload (the orchestrator's Stage-3 failure prompt surfaces it to the consultant).
...
- `framework/assets/wireframes/template-variant-landing.html` — variant landing scaffold; read at step 5.
...
- `framework/skills/verify-artifact-write.md` — write verification; invoked at step 3 (DS), step 4 (per screen), step 5 (landing + manifest + variant-position).
...
- `variant-position.json` — immutable mirror of variants.json own-entry, augmented:
    ...
    ```json
    {
      ...
      "strengths": [
        "18 fields visible in one viewport (no scroll for happy path)",
        "Keyboard-driven; no mouse required for the happy path",
        "Submit-on-Enter from any field"
      ],
      "weaknesses": [
        "Cognitive load high for first-time users",
        "Validation density may overwhelm in error case",
        "Tooltip discovery required to learn keyboard shortcuts"
      ],
      "tradeoffs": "Expert-first; low learnability.",
      "use_when": "Daily users, high volume; persona has used a similar product before."
    }
    ```
...
- `wireframes.html` exists and links the per-variant DS.
- `manifest.json` and `variant-position.json` both parse as valid JSON.
- The output_dir contains no files outside the documented set (`wireframe-ds.css`, `screen-NN-*.html`, `wireframes.html`, `manifest.json`, `variant-position.json`).
