...
- The `## Application character` section (when present) → in-memory `app_character` `{ name_and_statement, tone_attributes, per_surface_guidance }`. Absent, or the literal "none recorded" fallback → `app_character = null` (copy is phrased in a neutral professional voice).
