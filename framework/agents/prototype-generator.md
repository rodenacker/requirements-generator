...
- **Visual-craft floor (`visual-craft-standard.md`) satisfied on every surface**, checked by static sweep over everything authored this run (components + routes) — these are greps, not judgement calls:
  ...
  - **Press reachability:** every non-semantic clickable (an `onClick` on a `div`/`span`/`li`, or on a non-table `tr`) carries `data-pressable` (or `data-clickable` for a row). Without it the global press layer cannot reach it, so the element is silently untactile. A `<button>`/`<a>`/`<summary>` needs nothing.
  - **No clickable data-table row:** an `onClick`, `data-clickable` or `data-pressable` on a `<tr>`/`<TableRow>` inside a data table is a FAIL **outright** — the attribute does not redeem it. `design-system-standards.md §1 > Tables` forbids whole-row click; the identifier is a link/button in its own cell and row actions sit in a dedicated action column. The `tr[data-clickable]` affordance remains correct for row-like lists that are not data tables.
