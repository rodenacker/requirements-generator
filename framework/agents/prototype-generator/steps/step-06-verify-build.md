...
**Goal:** Prove the prototype lints, typechecks, builds, and clicks — the handback gate.
...
2. Branch on the verdict:
   ...
   - **`structured-fail {phase, summary}`** → **bounded retry** (budget N=2, tracked in memory):
     ...
     - Regenerate only that surface: re-run its sub-agent (step-04 for one surface) and/or re-compose its route (step-05), or fix the driver-owned data layer if the error is there.
