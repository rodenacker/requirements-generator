...
- **Event schema** (one JSON object per line, append-only):
    ...
    `t` is an ISO-8601 UTC timestamp captured at write time. `stage` is always `"prd-drafter"`. `substep` is one of the seven names in the table below. `run_id` is propagated per the rule below.
...
- **Instrumented sub-steps** (in workflow order; emit `substep_start` immediately before the substep's first action and `substep_end` immediately after its last successful action):
    ...
    | `populate-template` | Steps 2–3 | immediately after `read-inputs`'s `substep_end` | after the top-to-bottom template population pass completes, before `grep-crosscheck` begins |
    ...
    | `write-claims-sidecar` | Step 6a | before writing `prd/draft-claims.ndjson` | after `verify-artifact-write` for `prd/draft-claims.ndjson` returns `pass` |
...
- `framework/skills/verify-artifact-write.md` — read-back / hash-check called immediately after every Write.
...
- Do not skip `framework/skills/verify-artifact-write.md` after any Write.
...
- Do not skip **Workflow** steps 6a (sidecar emission + verify-artifact-write) or 6b (`grounding-verifier`). The drafter-handoff gate refuses to advance until `failed: 0`.
