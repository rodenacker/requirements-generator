...
- The row's fingerprint comparison target, per `map_row.fingerprint_compares_to`: `requirements/source-manifest.json` (**bytes-for-sha256 only**, at Step 2, only if it exists; content never parsed) for `source-manifest` rows, or `requirements/requirements.md` for `requirements-doc` rows (bytes-for-sha256 at Step 2; a **bounded content read at Step 5** — only the selected findings' cited sections plus the `## Amendments (pending re-merge)` section — for `elicitation-with-options` rows only, to ground the candidate enrichments; full content read only at Step 9b, and only on the consultant's addendum opt-in). The Step-5 read is a **scoped exception** to the hash-only rule: it never widens to a whole-file read, and never applies to any other `resolution_semantics`.
...
- **Elicitation-with-options flows** (`map_row.resolution_semantics == elicitation-with-options` — the question-style review methodologies, where each finding is an open gap an experienced BA/UX peer would raise): act as a **critical senior BA/UX peer and strategic thinking partner**, not an answer-collector. There is **no "Accept all remaining as drafted"** for this flow — each gap has multiple distinct candidates, so every finding needs its own choice. Per finding:
    ...
    2. Draft **≥2 (target 3) genuinely distinct candidate enrichments** spanning the realistic decision space — never one real option padded with strawmen. Each candidate is a declarative, corpus-ready resolution statement (what `requirements.md` would then assert) + a one-line **Implication** (what choosing it commits the design/build to) + **exactly one grounding tag** (canonically defined in `framework/assets/resolve-review/template-resolutions.md`): `[grounded: <anchor>]` (a §N.N / F-NN / BR-NN / existing AMD-NN from the Step-5 grounding read), `[domain-default]` (an enterprise financial-transaction data-management convention — maker-checker, four-eyes, SLA cut-offs, reconciliation consumers, …), or `[assumption — confirm with client]`. **Reconcile** every candidate against the grounding read's `## Amendments (pending re-merge)` entries and any prior finalized resolutions bearing on the gap, flagging tensions (e.g. a candidate needing a data field already scoped out). A candidate needing a new §7 data-shape property or F-NN parameter states it as a **proposed addition**, never asserts it as existing (closed-property-set rule, `CLAUDE.md` §2).
...
- `requirements/requirements.md` — a **bounded** content read at Step 5 (the selected findings' cited sections + the `## Amendments (pending re-merge)` section), `elicitation-with-options` rows only.
...
- The fingerprint comparison target was hashed at most once and its content never parsed at Step 2. `requirements/requirements.md` content is read only at Step 9b (addendum opt-in) and — for `elicitation-with-options` rows only — in the bounded Step-5 grounding read (the selected findings' cited sections + the `## Amendments (pending re-merge)` section, never the whole file). No file under `input/`, no sibling review artefact, and nothing under `framework/state/` or `framework/shared/` was read.
- **Elicitation-with-options flows (when the row uses them):** every finding resolved under this flow offered **≥2 genuinely distinct** candidate enrichments; every candidate carried **exactly one** grounding tag (`[grounded: …]` / `[domain-default]` / `[assumption — confirm with client]`); every `[AI-INFERRED, CONSULTANT-CONFIRMED]` resolution maps to that finding's **individual** candidate selection (no bulk, no accept-all); no candidate asserted a new §7 data-shape property or F-NN parameter as already existing (proposed additions are flagged as such); out-of-scope was reachable only via the free-text path and was never a prominent default option; and every resolution block from this flow carries its grounding tag on a Grounding line.
...
- Do not parse the Step-2 fingerprint comparison target. Hash-only; the drift check needs bytes, not content. (`requirements/requirements.md` content is read only at Step 9b on the opt-in, and — for `elicitation-with-options` rows — in the bounded Step-5 grounding read.)
- Do not widen the Step-5 grounding read beyond the selected findings' cited sections + the `## Amendments (pending re-merge)` section, and do not perform it for any row other than `elicitation-with-options`. It is a scoped exception, not a licence to read the whole document for other semantics.
...
- Do not assert a new §7 data-shape property or F-NN parameter as existing inside a candidate enrichment — state it as a proposed addition (closed-property-set rule). Do not offer "Accept all remaining as drafted" for `elicitation-with-options` (each gap needs its own choice among distinct options); a selected candidate stays `[AI-INFERRED, CONSULTANT-CONFIRMED]` (never laundered to `[CONSULTANT-STATED]` on enthusiasm), and only typed/edited content — including a typed "out of scope" — earns `[CONSULTANT-STATED]`.
