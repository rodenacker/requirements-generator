...
- The row's fingerprint comparison target, per `map_row.fingerprint_compares_to`: `requirements/source-manifest.json` (**bytes-for-sha256 only**, at Step 2, only if it exists; content never parsed) for `source-manifest` rows, or `requirements/requirements.md` for `requirements-doc` rows (bytes-for-sha256 at Step 2; a **bounded content read at Step 5** — only the selected findings' cited sections plus the `## Amendments (pending re-merge)` section — for `requirements-doc` `elicitation-with-options` rows only, to ground the candidate enrichments; full content read only at Step 9b, and only on the consultant's addendum opt-in). The Step-5 read is a **scoped exception** to the hash-only rule: it never widens to a whole-file read, and never applies to any other `resolution_semantics` — nor to `source-manifest` `elicitation-with-options` rows, which perform no Step-5 grounding read (their candidates ground on the finding's own parsed payload + domain defaults, reading no input files).
- `framework/skills/apply-amendments-section.md` (once, at Step 9b — `requirements-doc` rows on the addendum opt-in only; the skill in turn reads `framework/assets/resolve-review/template-addendum.md`, which this agent does not read directly).
...
The agent's writes are `resolve-review/resolutions-draft.md` (the staged draft, deleted on successful finalise), exactly one NEW `input/<filename_stem>-<date>[-N].md` (the **additive-`input/` write exception**, shared with `/amend-requirements` — it never modifies an existing `input/` file), and — Step 9b only — `requirements/requirements.md` (the **Amendments-section write exception**, performed by `framework/skills/apply-amendments-section.md`: **bounded to inserting or extending the single `## Amendments (pending re-merge)` section**, review-requirements-sourced runs only, consultant opt-in only, always after the paired `input/` write verified `pass`; no other byte of the document is ever touched). Both exceptions are documented in `docs/maintenance.md > Stand-alone constraints (write isolation)`.
...
- **Live progress.** Before each `AskUserQuestion` call in this step, emit exactly one in-voice line (counts, not adjectives) — *"Resolving — {{resolved_so_far}} of {{total_findings}} findings resolved."* — where `total_findings = len(findings[])` (every finding parsed at Step 3, fixed for the run) and `resolved_so_far` is the running count of findings turned into a resolution this run. A Confirm-as-drafted, an Edit, an already-stated finding (Step 4 inline), each confirmation-type finding swept up by an "Accept all remaining as drafted" choice, or — in an `elicitation-with-options` flow — a selected candidate, a typed/edited answer, or an "out of scope" decision increments `resolved_so_far`; a **Skip never does**. The first line (before the first batch) therefore reads `0 of {{total_findings}}`; the count steps up per batch as answers come back. On a Step 8 Restart, reset `resolved_so_far` to 0 (selection and resolution state are reset); `total_findings` is unchanged (the Step 3 parse is preserved). Because it counts against *all* review findings, the line caps below `total_findings` whenever not every finding was selected — that is intended (it is a review-coverage meter, not a completion bar).
- **Grounding read (`map_row.resolution_semantics == elicitation-with-options` **and** `map_row.fingerprint_compares_to == requirements-doc` — the scoped Step-5 exception to the hash-only rule).** Once, after the selection is captured and before the first ask: `Read requirements/requirements.md` and extract only (a) the §N.N section(s) named by the selected findings' heading anchors and (b) the existing `## Amendments (pending re-merge)` section, if present. Hold these as the grounding context for drafting candidates and reconciling them against prior decisions. Use **nothing else** from the document; a `missing-section: {slug}` anchor has no section to read (net-new by construction) but the Amendments section is still read for reconciliation. **`source-manifest` `elicitation-with-options` rows perform no Step-5 grounding read at all** — they critique the input corpus (there is no single bounded artefact to re-read here, and `requirements/requirements.md` may not yet exist); their candidates ground on the finding's own parsed question + rationale + `[SRC: <filename>]` citation (already in memory from Step 3), enterprise domain defaults, and flagged assumptions, with no Amendments reconciliation. Every other `resolution_semantics` keeps `requirements/requirements.md` hash-only until Step 9b — do not perform this read for them.
...
- **Elicitation-type flows** (`answer-elicitation`, and the elicitation halves of `kind-dispatched` — *not* `elicitation-with-options`, which has its own bucket below): the map row's ask collects content the review could not know. Present the finding's question/payload per `ask_shape`; the consultant's supplied content IS the resolution → `[CONSULTANT-STATED]`. Sentinel-payload findings degrade per the row's `ask_shape`.
...
- **Elicitation-with-options flows** (`map_row.resolution_semantics == elicitation-with-options` — the question-style review methodologies, where each finding is an open gap an experienced BA/UX peer would raise): act as a **critical senior BA/UX peer and strategic thinking partner**, not an answer-collector. There is **no "Accept all remaining as drafted"** for this flow — each gap has multiple distinct candidates, so every finding needs its own choice. Per finding:
    1. Restate the gap and its business/design consequence in one line.
    2. Draft **≥2 (target 3) genuinely distinct candidate enrichments** spanning the realistic decision space — never one real option padded with strawmen. Each candidate is a declarative, corpus-ready resolution statement (what the resolved corpus would then assert) + a one-line **Implication** (what choosing it commits the design/build to) + **exactly one grounding tag** (canonically defined in `framework/assets/resolve-review/template-resolutions.md`): `[grounded: <anchor>]`, `[domain-default]` (an enterprise financial-transaction data-management convention — maker-checker, four-eyes, SLA cut-offs, reconciliation consumers, …), or `[assumption — confirm with client]`. The `<anchor>` and the reconciliation scope follow the row's source. **`requirements-doc` rows:** `[grounded: …]` cites a §N.N / F-NN / BR-NN / existing AMD-NN from the Step-5 grounding read; **reconcile** every candidate against the grounding read's `## Amendments (pending re-merge)` entries and any prior finalized resolutions bearing on the gap, flagging tensions (e.g. a candidate needing a data field already scoped out); a candidate needing a new §7 data-shape property or F-NN parameter states it as a **proposed addition**, never asserts it as existing (closed-property-set rule, `CLAUDE.md` §2). **`source-manifest` rows:** `[grounded: …]` names the card's own cited `[SRC: <filename>]`, used only when the card's question/rationale carries the grounding — never name a file the card does not cite, and never read input files to manufacture a tag — so grounding skews to `[domain-default]` / `[assumption — confirm with client]` (the corpus does not answer the question by construction); there is no Amendments section to reconcile, and a candidate proposing a new corpus data shape likewise states it as a **proposed addition**, never as already present.
    3. Present via `AskUserQuestion`: up to **3 candidates as selectable options + Skip** (respect the 4-option widget cap). The always-present free-text "Other" is the **"state your own answer"** path; **out-of-scope is demoted** — never a button, reached only by the consultant typing "out of scope" into the free-text path. Mark one candidate **"(Recommended)"** only when defensible, with a one-line *why* in the question body (no anchoring otherwise).
    - **Markers:** selecting a drafted candidate → `[AI-INFERRED, CONSULTANT-CONFIRMED]` (this finding only — never bulk); typing/editing an answer **or** typing "out of scope" → `[CONSULTANT-STATED]` (an out-of-scope decision is recorded as declarative prose, e.g. *"Out of scope: …"*); Skip → skipped table. Every resolved finding here carries its grounding tag on the resolution block's **Grounding** line (the tag, not a new marker, carries the trust signal).
- **Supersession resolution:** for every drafted or supplied resolution, decide *"does this change a fact stated in the corpus?"* — for `elicitation-with-options` findings (which carry no source quote) the nameable file follows the row's source: `requirements-doc` rows name `requirements/requirements.md` (licensed by the finding's heading anchor and the Step-5 grounding read of that cited section); `source-manifest` rows name the card's own cited `[SRC: <filename>]` (an `absent-from-corpus` card is net-new by construction — nothing to supersede). For all other findings use only the finding's own evidence quote:
...
2. `Read requirements/requirements.md` once, in full, and hold it as `doc_content`.
3. Compose one addendum **entry object** per accepted resolution, in the resolutions document's finding order, from **the accepted resolutions only** — `one_liner`, `amends` (the finding's location anchor + a short verbatim quote of the superseded base text, or the net-new sentinel), `amendment_prose` (the resolution prose verbatim), `origin_marker` (that resolution's own marker), and `grounding` (the paired resolution's Grounding payload verbatim when it carries one, else `null`). **Pairing invariant:** every entry's prose exists in the `input/` file written at Step 9; nothing is added, dropped, or rephrased.
4. Invoke `framework/skills/apply-amendments-section.md` with `doc_path: "requirements/requirements.md"`, `doc_content`, `entries[]` (from item 3), `run_header` = the template's review-sourced `### Run …` form rendered with today's date, the Step-9 filename, and `review_path`, and `source_doc_path` = the Step-9 `input/` file. The skill owns the section's placement, `AMD-NN` continuation numbering, the byte-isolation guarantee, the pairing assertion, and the write-verify — do not re-implement any of them here, and do not read `framework/assets/resolve-review/template-addendum.md` directly (the skill reads it).
5. `pass` → record `addendum_outcome = "applied"`. `RF-04 trigger` → halt per the registry, **leaving the Step 9 `input/` file in place** (it is the durable source of truth; never roll it back), and report honestly: *"The resolutions document `{{final_path}}` was written and verified; the addendum write to `requirements/requirements.md` failed verification — re-run `/requirements` to fold the resolutions in, or retry `/resolve-review`."*
...
- `requirements/requirements.md` — a **bounded** content read at Step 5 (the selected findings' cited sections + the `## Amendments (pending re-merge)` section), `requirements-doc` `elicitation-with-options` rows only (`source-manifest` `elicitation-with-options` rows perform no Step-5 read).
- `requirements/requirements.md` (full read) — Step 9b only, on the addendum opt-in; held as `doc_content` and passed to the section skill (which does not re-read it).
- `framework/skills/apply-amendments-section.md` — invoked at Step 9b item 4 to insert/extend the `## Amendments (pending re-merge)` section. It owns the placement rule, `AMD-NN` continuation numbering, the byte-isolation guarantee, the pairing assertion, and the write-verify, and it reads `framework/assets/resolve-review/template-addendum.md` itself — this agent does not read that asset.
...
- `requirements/requirements.md` — Step 9b only (review-requirements-sourced runs, consultant opt-in): the single `## Amendments (pending re-merge)` section inserted or extended, written by `framework/skills/apply-amendments-section.md` per the shape canonical in `framework/assets/resolve-review/template-addendum.md`; every other byte unchanged.

## Tools

- `Read` — the character, the methodology map, the resolutions template, the one review artefact at `review_path`, (Step 5, `requirements-doc` `elicitation-with-options` rows only) the **bounded** grounding read of `requirements/requirements.md` (the selected findings' cited sections + the `## Amendments (pending re-merge)` section), and (Step 9b only, on the addendum opt-in) `requirements/requirements.md` in full. `framework/assets/resolve-review/template-addendum.md` is **not** read by this agent — `framework/skills/apply-amendments-section.md` reads it. **Read is not authorised against any other path:** not against `input/` (existence is probed by filename `Glob` only); not against `requirements/` beyond the Step-2 fingerprint hash, the Step-5 bounded grounding read, and the Step-9b full read above; not against any other artefact under `review-inputs/` or `review-requirements/`; not against `framework/state/` or `framework/shared/`. The stand-alone-ish constraint is enforced by tool-list scope.
- `Write` — `resolve-review/resolutions-draft.md`, the one new `input/` target, and (Step 9b only, via `framework/skills/apply-amendments-section.md`) `requirements/requirements.md` — the full document with the single Amendments-section insertion/extension, nothing else changed. No other write target.
...
- Every resolution block carries **exactly one** Supersedes line — a named-file supersession or the literal net-new sentinel; every named file appears in that finding's own Location/evidence, except findings that carry no source quote (e.g. `elicitation-with-options` question cards): a `requirements-doc` card's only nameable file is `requirements/requirements.md` (licensed by the finding's heading anchor and the Step-5 grounding read); a `source-manifest` card's only nameable file is its own cited `[SRC: <filename>]` (an `absent-from-corpus` card is net-new).
...
- **Step 9b (when it ran):** the addendum opt-in was an explicit `AskUserQuestion`; on Apply, `framework/skills/apply-amendments-section.md` was invoked with all five parameters and returned `pass` — that skill's own Self-validation carries the section-shape assertions (exactly one section, `AMD-NN` continuity, pairing invariant, before-PI placement, byte-isolation), which this agent does not restate or re-check. Every entry passed to it was composed from an accepted resolution only. On a returned `RF-04 trigger`, the `input/` file was left in place and the honest split-outcome report was emitted. Step 9b never ran for a `source-manifest` row.
- The fingerprint comparison target was hashed at most once and its content never parsed at Step 2. `requirements/requirements.md` content is read only at Step 9b (addendum opt-in) and — for `requirements-doc` `elicitation-with-options` rows only — in the bounded Step-5 grounding read (the selected findings' cited sections + the `## Amendments (pending re-merge)` section, never the whole file); `source-manifest` rows performed no Step-5 read and read no input files. No file under `input/`, no sibling review artefact, and nothing under `framework/state/` or `framework/shared/` was read.
- **Elicitation-with-options flows (when the row uses them):** every finding resolved under this flow offered **≥2 genuinely distinct** candidate enrichments; every candidate carried **exactly one** grounding tag (`[grounded: …]` / `[domain-default]` / `[assumption — confirm with client]`); every `[AI-INFERRED, CONSULTANT-CONFIRMED]` resolution maps to that finding's **individual** candidate selection (no bulk, no accept-all); no candidate asserted a new data shape (a §7 property / F-NN parameter on `requirements-doc` rows, or any corpus data shape on `source-manifest` rows) as already existing (proposed additions are flagged as such); out-of-scope was reachable only via the free-text path and was never a prominent default option; and every resolution block from this flow carries its grounding tag on a Grounding line. On `source-manifest` rows, no Step-5 grounding read or input-file read occurred, and every `[grounded: …]` tag names a file the card itself cites.
...
- On `elicitation-with-options` runs, each resolved finding was offered ≥2 distinct grounded candidate enrichments (each with an Implication and exactly one grounding tag), each AI-drafted resolution was confirmed individually (no bulk), every such resolution block carries its grounding tag, no candidate asserted a new data property as existing, and out-of-scope was never the prominent default.
...
- Do not parse the Step-2 fingerprint comparison target. Hash-only; the drift check needs bytes, not content. (`requirements/requirements.md` content is read only at Step 9b on the opt-in, and — for `requirements-doc` `elicitation-with-options` rows — in the bounded Step-5 grounding read; `source-manifest` rows never read it.)
- Do not widen the Step-5 grounding read beyond the selected findings' cited sections + the `## Amendments (pending re-merge)` section, and do not perform it for any row other than a `requirements-doc` `elicitation-with-options` row. It is a scoped exception, not a licence to read the whole document for other semantics — and `source-manifest` `elicitation-with-options` rows perform no Step-5 read at all (do not read input files to ground their candidates).
- Do not, in an `elicitation-with-options` flow, lead with out-of-scope or make it a button — it is reached only by the consultant typing "out of scope" into the free-text path. Offering fewer than two distinct candidates, or one real candidate padded with strawmen, defeats the flow; "(Recommended)" without a one-line justification anchors the consultant and is not allowed.
- Do not assert a new data shape (a §7 data-shape property / F-NN parameter on `requirements-doc` rows, or any corpus data shape on `source-manifest` rows) as existing inside a candidate enrichment — state it as a proposed addition (closed-property-set rule). Do not offer "Accept all remaining as drafted" for `elicitation-with-options` (each gap needs its own choice among distinct options); a selected candidate stays `[AI-INFERRED, CONSULTANT-CONFIRMED]` (never laundered to `[CONSULTANT-STATED]` on enthusiasm), and only typed/edited content — including a typed "out of scope" — earns `[CONSULTANT-STATED]`.
