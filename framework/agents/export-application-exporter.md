...
3. **Construct the export in memory.** The source document is the carrier; apply only these transforms, top to bottom — **transform, never re-draft**:
    ...
    - **3f. Everything else** — including §1.7, §6.6.1, §6.6.2 (with their scope-note blockquotes), the §6.1 `Rationale` column, and all `[SRC: C-NNN]` tags — passes through **byte-identical**.
