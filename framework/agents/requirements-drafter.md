...
6. *(Timing — emit batched `substep_end[gap-pass]` + `substep_start[derive-architectural-implications]` in one tool call before authoring §1.7. The timing events fire on **every** run to preserve the nine-substep contract and the paired-adjacent batching idiom, even when the step is a no-op under prototype.)* **Derive architectural implications.** **Gate on target: §1.7 is emitted under `manifest_target == "application"` only** (template §0.1). Under `manifest_target == "prototype"`, §1.7 is omitted — this step performs no §1.7 emission (a no-op wrapped by its timing events) and you skip directly to step 7. Under `manifest_target == "application"`: walk §6.1 functional requirements + §10 volumes + §6.7 reporting rows against the inline capability catalogue under **Architectural-implications catalogue** below. For each active category, emit one §1.7 row: `Capability category` = the catalogue category name; `Driving requirement(s)` = concatenated cross-refs (`→ §6.1 F-NN`, `→ §10`, `→ §6.7 RPT-NN`); `Recommendation` = populated **only** when a deterministic guidance applies (e.g. "≤10⁴ records → in-memory acceptable"), otherwise blank. Every row carries `[AI-SUGGESTED: AI-NNN | non-blocking]` — resolver Q&A refines. Stack picks (frameworks, vendors, products, versions) are forbidden in every cell per `GR-20`; the drafter speaks in capability categories only. This step must run **after** the gap pass so §6 fabrications are visible, and **before** the Mermaid step so the body stabilises before the diagram is authored.
...
- **Instrumented sub-steps** (in workflow order; emit `substep_start` immediately before the substep's first action and `substep_end` immediately after its last successful action):
    ...
    |---|---|---|---|
    | `read-inputs` | Step 1 | before reading `requirements/source-manifest.json` | after every manifest-registered file has been Read into context (every Native row's `original_path` and every Supported-via-MCP row's `converted_sibling`) |
...
**Rationale column (§6.1, application only).** Under `manifest_target == "application"` the optional §6.1 `Rationale` cell carries the business "why"; cite it `[SRC: C-NNN]` when input-stated, else `[AI-SUGGESTED: non-blocking]` when inferred, else leave the cell empty (it is optional). Under `manifest_target == "prototype"` the column is omitted (template §0.1).
...
- §1.7 (application target only — omitted under prototype) every cell **except** the optional `Recommendation` column (a deterministic guidance phrase is not a quotable source claim).
...
- §6.1 every F-NN row's `Statement` **and** `Acceptance criteria` cells (AC in EARS form per `GR-23`). The `Priority` cell and the `Rationale` cell (application target) only when input-stated (priority otherwise `[STANDARD-RULE: GR-24]`; rationale otherwise `[AI-SUGGESTED]` or empty).
...
- **§1.7 driving-requirement coverage (Tier B4):** applies under `manifest_target == "application"` only (§1.7 is omitted under prototype, and this check is skipped there). When emitted, every §1.7 row's `Driving requirement(s)` cell cross-references at least one §6.1 / §6.7 / §10 row that exists in the draft. Dangling cross-refs are a remediate-and-re-run condition.
...
- **Conditional-section emit predicates:**
    ...
    - §1.7, §6.6.1, §6.6.2 are present **iff** `manifest_target == "application"` (omitted under prototype per template §0.1). The §6.1 `Rationale` column is present **iff** `manifest_target == "application"`.
...
- Do not leave fields blank in an **emitted** section — when inputs are silent, walk the **Classification** decision tree to apply the correct marker. This does **not** override the emit-conditional / content-conditional rules: §1.7 / §6.6.1 / §6.6.2 (under prototype) and §1.6 / §8 / §9 (when their content predicate is false) are **omitted entirely**, not filled with blank or filler rows.
...
- Do not skip **Workflow** step 6 (`derive-architectural-implications`) **under `manifest_target == "application"`** — §1.7 must be populated on every clean application run (if no catalogue category matches, §1.7 still emits `Client-side state management`, which always matches). Under `manifest_target == "prototype"`, §1.7 is intentionally **absent** (template §0.1) and step 6 is a no-op — but still emit the step's `substep_start` / `substep_end` timing events to preserve the nine-substep contract and the paired-adjacent batching idiom.
...
- Do not emit conditional sections when their emit-predicate is false. This covers: content-conditional §2.5 / §6.9 / §7.X / off-mode §6.10 sub-block / §1.6 / §8 / §9, and target-conditional §1.7 / §6.6.1 / §6.6.2 + the §6.1 `Rationale` column (omitted under prototype). The drafter's self-validation does not require their presence in those cases.
