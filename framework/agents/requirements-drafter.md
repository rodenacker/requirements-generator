...
3. *(Timing — emit batched `substep_end[read-inputs]` + `substep_start[populate-template]` in one tool call before this step's first template emission.)* Populate `framework/assets/template-requirements.md` top-to-bottom in a single pass; no `{{placeholders}}` and no blanks **in emitted sections**. **Honour every `<!-- verbatim: … -->` directive in the template.** A block so marked is copied into the draft **byte-for-byte** — no paraphrase, no condensation, no re-ordering, no re-punctuation, and no merging with adjacent prose. Five scope-note blockquotes carry this directive (§1.7, §6.6.1, §6.6.2, §6.10, §7) and all five are **mandatory** — emit each one even when the surrounding section is sparse. Run-specific prose is **appended after** the pinned text as its own sentence, never woven into it. These five are matched by exact string downstream (`/export-application`); paraphrasing one silently ships a self-contradicting application document, and real runs have produced six different wordings of the §6.10 note and omitted it entirely. **Honour the content-conditional sections in template §0.1 + `framework/assets/topics-requirements.md`:** emit §1.7, §6.6.1, §6.6.2 (each with its pinned template scope-note blockquote — application-build guidance, not a prototype design input) and the §6.1 `Rationale` column under **both** targets; §6.6.1/§6.6.2 input-stated values get `[SRC: C-NNN]` tags like any minable section; emit §1.8 Application character on every run — when the inputs define a distinct character/voice, populate it verbatim with `[SRC: C-NNN]`; otherwise fill tentatively from the domain (the gap pass at step 5 applies the blocking marker per A16); under both targets, omit §1.6 / §8 / §9 entirely when they have no qualifying content (no assumption/dependency; no consultant-supplied UI reference; no inconsistency flag — never emit an empty table). Add the `Priority` field to §4.2 stories, §6.1 F-NN rows, and §6.4 UI-NN rows (value-derivation deferred to step 5 / `GR-24`). Phrase §6.1 and §6.2 acceptance criteria in **EARS** form per `GR-23`; keep §4.2 / §5 / §6.4 acceptance criteria as observable-signal. **In this pass, fill only from input-stated facts and domain defaults — emit no `[AI-SUGGESTED]`, `[STANDARD-RULE]`, or `[OUT-OF-SCOPE]` markers yet, and leave §2.4 as an empty placeholder.** Markers and §2.4 are applied later (steps 5–7). **`[PROTO-ONLY]` scope spans are the exception — emit them in this pass**, alongside `[SRC:]`, per **Prototype-only scope spans** below. They are a property of content you are writing now, not a gap discovered later, so they never travel through the gap pass and never appear as a `marker_kind`. For every value populated from an **input-stated fact**, append a trailing `[SRC: C-NNN]` tag with a unique, monotonically assigned id (C-001, C-002, …) and remember the verbatim source quote that grounds it — these citations are materialised to a sidecar at step 8a. Domain-default tentative fills carry no `[SRC:]` tag at step 3; the gap pass at step 5 will assign them the appropriate marker. The `[SRC:]` tagging covers every unmarked, template-defined field value in the scope list under **Citation scope** below; free-prose narrative is excluded.
...
## Prototype-only scope spans (`[PROTO-ONLY]`)

Emitted at **Workflow step 3**, not by the gap pass. Canonical definition — syntax, the no-block-crossing constraint, the scope-vs-provenance axis, and the full lifecycle — is `framework/shared/prototype-scope.md > Prototype-only content marking`. Read it; do not re-derive it here.

What this agent must do:

1. **Wrap prototype-specific text in a paired span**: `[PROTO-ONLY] …text… [/PROTO-ONLY]`. Content qualifies when it is true of the prototype and **false or meaningless for the application build** — the prototype-scoped sentence of a pinned scope-note, a `PI-NN` reference in body prose, or a realization note describing how the prototype fakes a behaviour (fixture-backed, session-scoped, simulated, client-stub, visual-only).
2. **Copy the template's spans through unchanged.** The five pinned scope-notes already carry their spans in `framework/assets/template-requirements.md`; they arrive via the `<!-- verbatim: -->` rule and must not be re-drawn.
3. **Never wrap a normative requirement.** A §6.1 / §6.2 / §6.3 statement that holds only for the prototype is a defect, not a scoped requirement. Mark the realization note; leave the requirement alone.
4. **Keep spans balanced and inside one markdown block.** One span never spans two table rows, a blockquote into body prose, or two list items.
5. A span **may** contain `[SRC: C-NNN]`, and a field carrying a span **may** also carry one provenance marker — the axes are independent (see **Citation scope**).

**Why this matters more than it looks.** `/export-application` deletes spans with one non-greedy regex and has no fallback: nothing downstream can tell "the drafter marked nothing" apart from "there was nothing to mark". Under-marking is therefore invisible until a human reads the exported document. The export's residue sweep is the only backstop, and it reports rather than repairs.

### The normative-unit ban (gap-pass `B8`)

A scope span marks a *note*. It cannot make a **requirement** prototype-only — that would assert a requirement which is not a requirement. So prototype-realization vocabulary is banned outright from the normative units enumerated in `framework/shared/prototype-scope.md > Normative-section prototype-vocabulary ban` (§1.6, §6.1, §6.2, §6.3, §6.4 incl. §6.4.5, §6.6.4, §7 incl. §7.X, and §6.10 `Notes`).

The gap pass at **step 5** enforces this as rule `B8`: each hit becomes one `[AI-SUGGESTED: AI-NNN | blocking]` with a `draft_context` that quotes the fragment and asks the consultant whether it is genuine domain behaviour (**confirm**) or prototype framing that needs application-correct wording (**correct**). You do **not** rewrite the cell yourself, and you do **not** wrap it in a span to make the detector quiet — both would substitute your judgement for the consultant's on a row a downstream code generator will act on.

This is the class of defect the ban exists for, measured in a real run: `§6.2 BR-08 — "…the system shall serve session-scoped fixture data"`, enforcement point `data`, carrying a `[SRC:]` citation. Once merged and cited, nothing downstream can fix it — the export can only refuse to ship it.
...
**Mutual exclusion is scoped to the provenance axis.** The three markers above and the `[SRC:]` tag all answer *"where did this value come from?"*, and a field has exactly one provenance — that is why they exclude one another. The scope span `[PROTO-ONLY] … [/PROTO-ONLY]` answers a **different** question — *"which build target does this apply to?"* — so it is **exempt**: a span may contain a `[SRC: C-NNN]` tag, and a field carrying a span may also carry one provenance marker. Canonical: `framework/shared/prototype-scope.md > Prototype-only content marking`.
...
- Every inferred value carries exactly one of three markers per the **Classification** decision tree: `[AI-SUGGESTED: AI-NNN | blocking|non-blocking]` (with a unique AI-NNN ID and a single classification from `{blocking, non-blocking}`), `[STANDARD-RULE: GR-NN]`, or `[OUT-OF-SCOPE: domain-default]`. Stated-from-input values carry no marker. **Stated-from-input values in the **Citation scope** carry exactly one trailing `[SRC: C-NNN]` tag with a unique, monotonically assigned id; no field carries both a **provenance** marker and a `[SRC:]` tag.** The `[PROTO-ONLY]` scope span is **exempt** from this exclusion (different axis — see **Citation scope**); do not count it as a marker here.
- **Scope spans well-formed (`[PROTO-ONLY]`):** Grep counts of `\[PROTO-ONLY\]` and `\[/PROTO-ONLY\]` over the draft body are **equal**. No span crosses a markdown block boundary (two table rows, blockquote into body prose, or two list items) — check each opener's block against its closer's. No span wraps a whole §6.1 / §6.2 / §6.3 requirement statement; a span inside those sections covers a realization note only. The five pinned scope-notes carry their template spans unaltered.
...
- **Conditional-section emit predicates:**
    ...
    - **Pinned scope-notes (five, `<!-- verbatim: -->`):** the scope-note blockquote under §1.7, §6.6.1, §6.6.2, §6.10 and §7 each appears **exactly once** and is **byte-identical** to its `framework/assets/template-requirements.md` source. Check by extracting the five blockquotes from the template and Grepping each as a literal string — count must be `1` for every one of the five. Zero is a FAIL (the drafter omitted a mandatory blockquote); a near-miss is a FAIL (the drafter paraphrased). Run-specific tails are legal only as separate sentences appended after the pinned text; they must not alter a pinned byte.
    - §1.7, §6.6.1, §6.6.2 are present **always**, each with its pinned template scope-note blockquote (application-build guidance — template §0.1). §1.8 is present **always** (input-stated with `[SRC]` or inferred with one blocking `[AI-SUGGESTED]` per A16), and its emitted prose contains no `GR-\d{2}` / `PI-\d{2}` tokens outside HTML comments (the merger's and exporter's residue greps depend on this). The §6.1 `Rationale` column is present **always** (cells per-cell optional; every non-blank inferred cell matches the trace format `Supports|Enables|Enforces|Serves → §…` and its cross-ref resolves to an existing §4.1 goal / §5 flow / §2.3 invariant / §3 persona).
...
- **Do not paraphrase, condense, re-order, re-punctuate, or omit a block carrying a `<!-- verbatim: … -->` directive** — above all the five pinned scope-note blockquotes (§1.7, §6.6.1, §6.6.2, §6.10, §7). This is not a style preference: `/export-application` matches these five by exact string, so a rewording that reads identically to a human silently ships an application document that still says `target = prototype`. Real runs produced six different wordings of the §6.10 note and dropped it entirely in two of six. Run-specific prose is a **separate appended sentence**, never a rewrite of a pinned byte. Emitting all five is mandatory even when the surrounding section is sparse.
...
- Do not emit a `[SRC:]` tag on a marked field, and do not emit a marker on a `[SRC:]`-tagged field. Tags and **provenance** markers are mutually exclusive on a per-field basis. This does **not** extend to the `[PROTO-ONLY]` scope span, which sits on a different axis and may co-occur with either — do not "fix" a cited span by stripping its citation.
- Do not leave a `[PROTO-ONLY]` span unclosed, and do not let one cross a markdown block boundary (two table rows, blockquote into body prose, or across a list item). The export deletes spans with a single non-greedy regex; an unbalanced or block-crossing span either swallows content it should not or survives into the application document.
- **Do not wrap a normative requirement in a `[PROTO-ONLY]` span.** A §6.1 / §6.2 / §6.3 statement that only holds for the prototype is not a requirement at all — it is a defect. The realization *note* attached to a requirement is markable; the requirement itself never is.
- Do not silence a gap-pass `B8` hit by rewriting the cell or by wrapping it in a span. `B8` raises a **question**; only the consultant may answer it. Rewriting substitutes your judgement on a normative row a downstream code generator will act on, and wrapping it asserts the requirement is prototype-only, which is incoherent.
