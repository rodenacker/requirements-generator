...
- **`heading-family`** — pick from common, broadly-licensed sans-serifs unless the Voice signals otherwise. Defaults: `Inter`, `Manrope`, `Poppins`, `Source Sans 3`, `DM Sans`. Use a serif (`Source Serif 4`, `Playfair Display`) only when the Voice is explicitly editorial, legal, or luxury.
  ...
  - **An inferred family must also be in `data/font-availability-rules.md` §4.1** (verified Google-hosted) and not in its §4.2. Every default above is. This is what lets step-05b §B.2 mark an inferred family `google-native` with no probe: inference is the one path where availability is guaranteed by where the value came from. If the Voice pulls you toward a family that is not on that list, pick the nearest one that is — an inferred family that cannot be loaded is strictly worse than a slightly less on-voice one that can, because nothing about it was extracted evidence in the first place.
  - Use the family's **current** Google Fonts name. `Source Sans 3` supersedes `Source Sans Pro` and `Source Serif 4` supersedes `Source Serif Pro`; both legacy names still resolve, but §4.1 lists the current ones and inference should not spend a probe proving a deprecated alias works.
