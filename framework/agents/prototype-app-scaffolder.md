...
- `design-system/design-system-light.html` (or `-dark.html`, or the legacy unsuffixed `design-system/design-system.html`) — optional brand source (a); resolved and read by `extract-brand-theme.md` only. A `/design-system` run may have produced only one mode.
