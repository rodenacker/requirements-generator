...
- **`layout.tsx` carries the brand webfont links** and `.scaffold.json` records `brand_fonts` + `theme_contract: 2`. Verify the font actually resolves rather than assuming: the family is declared in `theme.css` either way, so this is invisible to lint, typecheck and the smoke.
