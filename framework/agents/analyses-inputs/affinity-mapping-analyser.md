...
0. **In plain terms** (`<section id="plain-terms">`) — `{{PLAIN_SUMMARY}}`: a 2–5 sentence plain-English lead (what this affinity map is, what it found, what the consultant should do with it). The first section, above the compact overview. Per `framework/shared/output-readability.md` (operative rules restated in the character's *Reader & plain language* block, so no `framework/shared/` read is needed).
1. **Compact overview** (`<header id="overview">`) — title, subtitle, meta-grid (domain, target, generated-at, manifest sha256, run number, note / cluster / super-theme / orphan / tension / drifted-note / consumed-row / skipped-row / irrelevant-row counts), and a `<nav class="jump-links">` linking to `#plain-terms`, `#diagram-primary`, `#diagram-tensions`, `#source-roster`, `#clusters`, `#orphans`, `#affinity-map-body`, `#diagnostics`.
2. **Primary mindmap** (`<section id="diagram-primary">`) — `{{PRIMARY_MINDMAP_SVG}}` (pre-rendered inline `<svg>` hub-and-spoke) above `{{PRIMARY_MINDMAP_MERMAID}}` (adjacent collapsed `<details class="mermaid-block">` with the `mindmap` export source). **The "MUST contain a diagram" deliverable.** Root → L3 super-themes → L2 clusters only; note-level leaves excluded.
3. **Tensions diagram** (`<section id="diagram-tensions">`) — `{{TENSIONS_BLOCK}}`. When `tensions.length >= 1` this is a `<figure class="affinity-tensions">` inline-SVG directed graph above a collapsed `<details class="mermaid-block">` containing the secondary `flowchart TD` export source; when `0` this is `<p class="no-tensions">No cross-cluster tensions surfaced in the corpus.</p>` (deterministic copy so the header is always structurally present).
...
| (operational) | Step 11 — Render (inline SVG) + write + verify | Compute mindmap + conditional tensions SVG geometry; emit `mindmap` / `flowchart TD` Mermaid export adjuncts; SHA-256; Write; verify-artifact-write; svg-overlap-check |
...
9. **Diagram validity.** Deferred to Step 11 (node coverage in sub-step C; geometric overlap in sub-step F). Every L3 super-theme and every L2 cluster appears as a node in the pre-rendered inline mindmap SVG (and in the `mindmap` export source); no dangling references; `svg-overlap-check` returns `total: 0` (or any overlap is recorded as a layout warning). Capture `gate_9_status: deferred-step-11`.
...
**Sub-step B — Compose diagrams (inline SVG + Mermaid export).**

**B1 — Primary mindmap SVG (`{{PRIMARY_MINDMAP_SVG}}`).** Compute the geometry of a top-down three-tier column hub-and-spoke per the template's MINDMAP SVG SCHEMA and emit one inline `<svg viewBox="0 0 W H" role="img" aria-label="…">` inside `<figure class="affinity-mindmap">`:
- Layout constants: `PAD = 24`, `COL_W = 230`, `NODE_W = 200`, `ROOT_H = 40`, `SUPER_H = 46`, `CLUSTER_H = 54`, `TIER_GAP = 34`, `CLUSTER_GAP = 14` (px, in the viewBox coordinate frame).
- `S = super_themes.length`. Column `k` (0-indexed, `ST-NN` order) centre `col_cx[k] = PAD + COL_W*k + COL_W/2`. `W = max(PAD*2 + COL_W*S, NODE_W + PAD*2)`.
- Root: `<g class="node"><rect class="am-node am-root" rx="14"/>` centred at `(W/2, PAD + ROOT_H/2)`, size `NODE_W × ROOT_H`; label `<text class="am-text am-root-label">` = `domain` (manifest meta) or `"Affinity Map"`.
- Super-theme row at `super_y = PAD + ROOT_H + TIER_GAP`; super `k` is `<rect class="am-node am-super" rx="6"/>` centred at `(col_cx[k], super_y + SUPER_H/2)`, size `NODE_W × SUPER_H`; label `<text class="am-text am-super-label">` wraps ≤ 2 lines via `<tspan>` (truncate the insight statement ≤ 60 chars at a word boundary, append `…` if truncated).
- Clusters under super `k` (in `TH-NN` order) start at `cy0 = super_y + SUPER_H + TIER_GAP`; cluster `j` is `<rect class="am-node am-cluster" rx="4"/>` centred at `(col_cx[k], cy0 + j*(CLUSTER_H + CLUSTER_GAP) + CLUSTER_H/2)`, size `NODE_W × CLUSTER_H`; label `<text class="am-text am-cluster-label">` wraps ≤ 2 lines via `<tspan>`.
- `maxC = max_k |clusters(k)|`; `H = cy0 + maxC*(CLUSTER_H + CLUSTER_GAP) - CLUSTER_GAP + PAD`.
- **Edges first** (drawn under nodes): one `<path class="am-edge am-edge-rs">` cubic from the root bottom-centre to each super-theme top-centre; per column, a **left-gutter bus** — one vertical `<path class="am-edge am-edge-sc">` spine at `spine_x = col_cx[k] - NODE_W/2 - 10` (LEFT of the node boxes, so it crosses no node) from the super-theme down to the last cluster, with a short horizontal connector from the spine into each cluster's left edge (endpoint-anchored). Never route a super→cluster line straight down the column centre through the stacked cluster boxes.
- All `<text>` XML-escaped. Gate 9 requires every super-theme and every cluster to appear as a node here.

**B2 — Tensions SVG (the lead figure of `{{TENSIONS_BLOCK}}`, only when `tensions.length >= 1`).** Emit one inline `<svg>` inside `<figure class="affinity-tensions">` per the template's TENSIONS SVG SCHEMA: lay the distinct tension-involved clusters out in one horizontal row of `<rect class="am-node at-node" rx="4"/>` nodes (label `<text class="at-text at-node-label">`, truncated insight statement); per tension, one curved `<path class="at-edge">` arc above the row from `from_cluster_id` to `to_cluster_id`, with the `description` as a `<text class="at-label">` at the arc apex over an opaque `<rect class="at-label-bg">`. All `<text>` XML-escaped. When `tensions.length == 0`, no figure is built (see Sub-step D).

**B3 — Mermaid export sources.** The `mindmap` and (when `tensions.length >= 1`) `flowchart TD` Mermaid sources below are the collapsed export / re-ingestion adjuncts — embedded as text inside `<pre class="mermaid-source">`, **not rendered in-page and not validated** (the inline SVG above is the visible diagram; the Mermaid source survives markitdown HTML→MD as a fenced ```mermaid block for `/requirements` re-ingestion).
...
When `tensions.length == 0`, neither the tensions SVG nor its Mermaid source is built; the template's `{{TENSIONS_BLOCK}}` placeholder receives the `<p class="no-tensions">No cross-cluster tensions surfaced in the corpus.</p>` literal block.

**Sub-step C — Diagram validity (Gate 9) — finalised after Sub-step F.**

Gate 9 is a structural + geometric check on the pre-rendered inline SVG; there is no Mermaid render dependency (`mmdc` is not required, and no `RF-07` is surfaced):

- **Node coverage.** Confirm every L3 super-theme and every L2 cluster appears as a node in the mindmap SVG (and as a node in the `mindmap` export source); no dangling references. On a mismatch, fix the SVG/source before write and re-check.
- **Overlap.** The geometric overlap pass runs in Sub-step F (after the artefact is on disk, which `svg-overlap-check` requires). Record `gate_9_status: deferred-step-11-F` here; finalise it to `pass` (overlap `total: 0`) or `overlap-warning` (overlaps recorded as diagnostics layout lines) once Sub-step F completes.
...
- `{{PRIMARY_MINDMAP_SVG}}` — the `<figure class="affinity-mindmap">` + inline `<svg>` composed in Sub-step B1.
- `{{PRIMARY_MINDMAP_MERMAID}}` — a **collapsed** `<details class="mermaid-block"><summary>Mindmap Mermaid source (export / re-ingestion adjunct)</summary><p class="mermaid-caption">Copy this block into <a href="https://mermaid.live">mermaid.live</a> or run <code>mmdc -i source.mmd -o diagram.svg</code> to view rendered. The pre-rendered SVG above is the in-page diagram; this Mermaid source survives markitdown HTML→MD as a fenced code block for /requirements re-ingestion.</p><pre class="mermaid-source">{{mindmap-source}}</pre></details>` — **not** `open` (collapsed by default).
- `{{TENSIONS_BLOCK}}` — when `tensions.length >= 1`, the Sub-step B2 `<figure class="affinity-tensions">` inline SVG followed by a collapsed `<details class="mermaid-block"><summary>Tensions Mermaid source (export / re-ingestion adjunct)</summary><p class="mermaid-caption">Copy this block into <a href="https://mermaid.live">mermaid.live</a> or run <code>mmdc -i source.mmd -o diagram.svg</code> to view rendered.</p><pre class="mermaid-source">{{tensions-source}}</pre></details>`. When `tensions.length == 0`, the `<p class="no-tensions">No cross-cluster tensions surfaced in the corpus.</p>` literal block.
...
- `{{DIAGNOSTICS_BLOCK}}` — pre-rendered `<section class="diagnostics">` per the template's DIAGNOSTICS SCHEMA. Contains:
  ...
  - `<h3>Diagram validity</h3>` + `<p class="diagram-status">` summary of Gate 9 (node coverage + `svg-overlap-check` result: `pass` or `overlap-warning`).
...
- If `svg-overlap-check` reported overlaps, append: *"Diagram: {n} SVG overlap(s) recorded as layout warnings in Diagnostics — the cluster cards remain the canonical deliverable and the Mermaid export is the clean fallback."*
...
- **Revise** — surface a follow-up `AskUserQuestion` asking which round(s) to re-run, per the revision-granularity table:
  ...
  - **Add a tension** ("there's a real tension between TH-07 and TH-09 I want surfaced"): append to `tensions[]`; re-build the tensions SVG + its Mermaid export source (which may now render where it didn't before); re-render; re-Write; re-verify; loop back to A.
...
- `framework/skills/svg-overlap-check.md` — invoked in Step 11 sub-step F (after verify-artifact-write `pass`).
...
**No MCP tools.** No Task delegation. The mindmap + tensions diagrams are pre-rendered inline SVG by the analyser directly — no `mmdc`, no Mermaid runtime, no external rendering pipeline. The Pass-2 sub-agent invocation is the only `Agent` call permitted.
...
- The artefact contains exactly one `<section id="plain-terms">` (the first section, carrying the "In plain terms" lead with a non-empty `<p>`), one `<header id="overview">`, one `<nav class="jump-links">`, one `<section id="diagram-primary">`, one `<section id="diagram-tensions">`, one `<section id="source-roster">`, one `<section id="clusters">`, one `<section id="orphans">`, one `<section id="affinity-map-body">`, one `<details id="diagnostics">`, one trailing `<section class="next-steps">`, and one `<details class="downstream-toggle">` (the collapsed "For downstream use" re-ingestion footer). DOM order is plain-terms → overview → diagram-primary → diagram-tensions → source-roster → clusters → orphans → affinity-map-body → diagnostics → next-steps → downstream-toggle.
- The `<section id="plain-terms">` `<p>` introduces no cluster, super-theme, count, or `[SRC]` not already present in the body — it is a condensation of the closed `model`, not a new source of facts. No client domain term is glossed in it; methodology jargon is glossed at first use (affinity cluster, super-theme, drifted note, insight-statement label, Pass-2 re-cluster).
...
- The `<section id="diagram-primary">` contains exactly one `<figure class="affinity-mindmap">` with one inline `<svg>` (carrying `role="img"` + a non-empty `aria-label`) followed by a **collapsed** `<details class="mermaid-block">` holding the `mindmap` export source in `<pre class="mermaid-source">`. Every L3 super-theme and every L2 cluster appears as a node (`<rect class="am-super">` / `<rect class="am-cluster">`) in the SVG.
- The `<section id="diagram-tensions">` contains either a `<figure class="affinity-tensions">` inline `<svg>` followed by a collapsed `<details class="mermaid-block">` with the `flowchart TD` export source (when `tension_count >= 1`), or the `<p class="no-tensions">No cross-cluster tensions surfaced in the corpus.</p>` (when `tension_count == 0`).
...
- All 10 quality-gate results are reported in the diagnostics block (either as `gate-pass`, `gate-fail`, or `gate-warn` lines, with Gate 9 possibly `overlap-warning` rendered as `gate-warn`).
...
- **Do not skip the primary mindmap.** The diagram-first ordering is the user's hard requirement. The mindmap is a pre-rendered inline `<svg>` computed by the analyser — there is no `mmdc` / Mermaid-render dependency and no `RF-07`; the diagram is always produced. The only sanctioned degradation is `gate_9_status: overlap-warning` when `svg-overlap-check` reports geometric overlaps (recorded as diagnostics layout warnings, never a halt).
- **Do not bundle a Mermaid renderer or require `mmdc`.** The `<pre class="mermaid-source">` blocks are copy-paste text + the re-ingestion adjunct; the inline `<svg>` is the rendered diagram. Do not add a client-side Mermaid runtime, a CDN, or any external rendering pipeline; do not validate the Mermaid source with `mmdc`.
