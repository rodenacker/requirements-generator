...
11. **Round-trip footer** (`id="round-trip"`) — static paragraph; the analyser does not edit it.
...
- `analyse-inputs/GLOSSARY/glossary.html` exists, has been verified, and contains a complete glossary: Overview, TOC, Domain terms, Application terms, the five open-item registers, JSON body block, Round-trip footer, Diagnostics (classification split + maturity histogram + proposal audit + discard log + Source roster + 10 gate results + Run history), and the `glossary-meta` cursor line.
