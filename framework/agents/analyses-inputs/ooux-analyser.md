...
The artefact has **seven visible surfaces** in DOM order: a compact overview header with counts and the manifest fingerprint; a source roster table (consumed + skipped manifest rows); a Mermaid `erDiagram` (the "MUST contain a diagram" deliverable — entities + relationships + cardinality); the canonical OOUX sticky-note column-board (one column per object — CTAs, header, CCPs, metadata, nested refs); a relationship matrix (tabular fallback); a `<pre><code class="language-json" id="ooux-object-map-body">` block carrying the full machine-readable object model (the load-bearing `/requirements` re-ingestion contract — survives markitdown HTML→MD as a fenced ```json code block); and a collapsed diagnostics block (synonym-merge log + 8 gate results + `irrelevant-to-domain` source rows + run history). The `<head>` also carries a small `<script type="application/json" id="ooux-object-map-meta">` block with counts and the manifest fingerprint for drift detection on subsequent runs (markitdown strips this block — it is not the round-trip carrier).
...
2. **Source roster** (`<section id="source-roster">`) — `{{SOURCE_ROSTER_BLOCK}}`: Consumed table (filename, tier, sha256[:8], nouns_contributed) + Skipped table (filename, reason).
3. **Object relationship diagram** (`<section id="diagram">`) — `{{ER_DIAGRAM_MERMAID}}`: a `<details class="mermaid-block">` containing the Mermaid `erDiagram` source in `<pre class="mermaid-source">`. **The "MUST contain a diagram" deliverable.**
4. **Object column-board** (`<section id="diagrams" class="object-board">`) — `{{OBJECT_COLUMNS}}`: one `<section class="object-column">` per object, five sticky stacks in fixed order (CTAs → header → CCPs → metadata → nested refs).
5. **Relationship matrix** (`<section id="tables">`) — `{{REL_MATRIX_BLOCK}}`: one row per recorded relationship.
6. **Object map body** (`<section id="object-map-body">`) — `{{OBJECT_MAP_JSON_BLOCK}}`: a single `<pre><code class="language-json" id="ooux-object-map-body">` carrying the full JSON object model per the reference's JSON schema. **The load-bearing markitdown-survival contract.**
...
| (operational) | Step 11 — Render + Mermaid validate + write + verify | Template substitution; Mermaid emission; mermaid-validator post-Write; SHA-256; Write; verify-artifact-write |
...
- State readiness in one short line: *"OOUX inputs-side analyser ready. Starting from `requirements/source-manifest.json`. Methodology: Sophia Prater's ORCA process adapted for raw consultant inputs — six rounds (Discovery → Objects → Relationships → CTAs → Attributes → CCPs); load-bearing synonym merge in Round 2; citations via `[SRC: <filename>]`; provenance markers `from-source-<filename>` / `synonym-merged-from-[<filenames>]` / `inferred-from-<filename>`; the artefact carries a Mermaid `erDiagram` plus the canonical sticky-note column-board plus an embedded JSON body block for `/requirements` round-trip; 8 hard quality gates including the inputs-side-specific Gate 8 (every consumed row contributes ≥ 1 candidate noun OR is marked irrelevant-to-domain)."*
...
- **If an attribute is a reference to another object on the map**, set the corresponding relationship's `also_nested = true` in the in-memory relationships list (this drives the nested-relationship sticky-note rendering plus the Mermaid erDiagram's PK / FK shape).
...
7. **Relationship matrix, nested references, and Mermaid erDiagram agree.** This is **deferred to Step 11 sub-step C** (after Mermaid composition) for the erDiagram leg of the check; the matrix-vs-nested-refs leg runs here. Capture `gate_7_status: partial-step-10` and finalise in Step 11.
...
**On any gate failure (1–6, 8 — Gate 7 finalises in Step 11):**
...
**Sub-step B — Compose Mermaid erDiagram source.**

Build the `erDiagram` source following the reference's "Mermaid erDiagram emission shape":

- Header line: `erDiagram`.
- For each relationship: one `<SOURCE> <cardinality-symbol> <TARGET> : "<label>"` line. Cardinality symbols: `||--||` for `1:1`, `||--o{` for `1:N`, `}o--o{` for `N:M`. (Mermaid's notation does not have a `N:1` variant; reverse the source/target for an `N:1` to render as `1:N` from the other side, preserving the directional semantics in the verb phrase.)
- For each object: one entity block `<ENTITY-NAME> { ... }`. Entity name is the canonical object name uppercased per Mermaid's convention; the original-case canonical name is preserved in the JSON body block.
- Inside each entity block:
  - First line: the primary CCP attribute, typed `string` (or `int`/`decimal`/`date`/`bool`/`enum` if the inputs name a type) with `PK` marker. Identifier-style attribute names get the type the inputs imply; default to `string` when no signal.
  - Subsequent lines: other CCPs in CCP order, then non-CCP attributes from Round 5.
- **Inferred objects** (`provenance = inferred-from-<filename>`): exclude from the erDiagram entirely (Mermaid syntax does not gracefully accommodate `<<inferred>>` decorators on entity headers); surface them in the diagnostics with a note that they are present in the column-board and JSON body but excluded from the erDiagram per the inputs-side convention. Record this exclusion in the diagnostics `mermaid_exclusions` list.

Example Mermaid output for the running example:
...
erDiagram
  ...
  ORDER_LINE }o--o{ PROMOTION : "applies"
  CUSTOMER {
  ...
  PROMOTION {
    string promotion_code PK
    string description
    decimal discount_amount
  }
```

(Note: `Wishlist` is omitted because its provenance is `inferred-from-<filename>` — the diagnostics' `mermaid_exclusions` list records the exclusion.)

**Sub-step C — Mermaid validation (Gate 9 leg, finalising Gate 7).**

Save the Mermaid source to a temporary file. Use a Bash `mkdir -p /tmp/ooux-vd` followed by writing the source to `/tmp/ooux-vd/ooux-erdiagram.mmd` (or the PowerShell equivalent on Windows-only environments).

- Invoke `framework/skills/mermaid-validator.md` with `file_path: /tmp/ooux-vd/ooux-erdiagram.mmd`.
...
- On syntax errors: apply the validator's auto-fix loop (the skill itself iterates). If the diagram cannot be made valid in three retries, drop the Mermaid: replace the `<pre class="mermaid-source">` content with `<!-- Mermaid erDiagram was rejected by the validator after 3 retries — see Diagnostics > Mermaid results -->`; record `[GAP-MERMAID-INVALID]` in diagnostics; do not let one bad diagram block the artefact. **The Mermaid drop does not relieve Gate 7's matrix-vs-nested check** — the matrix and nested-refs must still agree even when the erDiagram is dropped.
- On `pass`: also run the matrix-vs-nested-refs vs erDiagram leg of Gate 7: every relationship in the in-memory `relationships` list appears in the erDiagram with the same cardinality and same direction. Flag mismatches.

Update Gate 7's status: `pass` if matrix + nested + erDiagram all agree; `pass-with-drops` if the erDiagram was dropped but matrix + nested still agree; `fail` if any leg disagrees. On `fail`, surface `AskUserQuestion` Revise / Override / Restart per Step 10's branch (note that we are mid-Step-11 — the gate failure can still elect to halt the write).

**Sub-step D — Build substitution map.**

Every consultant-supplied string is **HTML-escaped** before injection (`<`, `>`, `&`, `"`, `'`). **Exception:** Mermaid source inside `<pre class="mermaid-source">` retains literal `|`, `[`, `]`, `{`, `}`, `||--o{` syntax — only the standard HTML escape set applies, not Mermaid-specific characters. The JSON inside `<pre><code class="language-json">` is written as a JSON-serialised string and rendered as text inside the `<pre><code>` block.
...
- `{{ER_DIAGRAM_MERMAID}}` — pre-rendered `<details class="mermaid-block">` containing the `<pre class="mermaid-source">` with the Mermaid source from Sub-step B (or the `<!-- ... drop comment -->` if Sub-step C dropped it).
...
- `{{DIAGNOSTICS_BLOCK}}` — pre-rendered `<section class="diagnostics">` per the template's DIAGNOSTICS SCHEMA. Contains:
  ...
  - `<h3>Quality gates (8)</h3>` + `<ul class="gate-results">` — one `<li class="gate-pass|gate-fail">Gate N — <description>.</li>` per gate in order 1–8 (Gate 7 may be `pass-with-drops` if the erDiagram was dropped). Override-only: append a sub-list under each failing gate naming the flagged items by id.
  ...
  - `<h3>Mermaid validation</h3>` + a one-line status: `pass` / `pass-with-drops` / `fail` (with the validator's error message if `fail`). On `pass-with-drops`, list excluded entities (the `mermaid_exclusions` list from Sub-step B).
...
- If `inferred_count > 0`, append: *"{inferred_count} objects inferred from context (provenance: `inferred-from-<filename>`) — these are listed in Diagnostics and excluded from the erDiagram; confirm or revise each."*
...
- **Revise** — accept the consultant's revision instructions in their next message:
  - **Reject a synonym merge** ("`Client` is distinct from `Customer` — separate them"): split the merge; reassign provenance markers; re-run gates 1, 4, 6; re-render (the column-board adds a column, the erDiagram adds an entity); re-validate Mermaid; re-Write; re-verify; loop back to A.
  - **Confirm an inferred object** ("`Wishlist` is real — interview-notes.md line 42 says 'customers maintain a wishlist'"): flip provenance from `inferred-from-<filename>` to `from-source-<filename>`; drop from the diagnostics' inferred list; include in the erDiagram; re-render; re-Write; re-verify; loop back to A.
  - **Drop an object** ("drop `Wishlist` — it's not part of the MVP"): remove the object + its CTAs / attributes / CCPs / relationships involving it; re-run gates 1, 4, 5, 7; re-render; re-validate Mermaid; re-Write; re-verify; loop back to A.
  - **Rename an object** ("rename `Customer` to `Account holder` per the brief at line 8"): update the canonical name + propagate to every CTA / attribute / CCP / relationship + every `[SRC]` chip; re-render; re-validate Mermaid; re-Write; re-verify; loop back to A.
  - **Add / remove / re-cardinality a relationship**: update the relationships list; re-run gate 3, 7; re-render; re-validate Mermaid; re-Write; re-verify; loop back to A.
...
- Transient: `/tmp/ooux-vd/ooux-erdiagram.mmd` during Step 11 sub-step C. Cleaned up implicitly at session end; not part of the deliverable.
...
- `Write` — write `analyse-inputs/OOUX/ooux-object-map.html`. Also write the transient Mermaid source file under `/tmp/ooux-vd/`.
...
- `Bash` — `mkdir -p analyse-inputs/OOUX` and `mkdir -p /tmp/ooux-vd` (Step 11 setup); the `mmdc` invocation is performed inside `framework/skills/mermaid-validator.md`, not directly by this agent. On Windows-only environments, use the PowerShell `New-Item` equivalent.
- `AskUserQuestion` — surface the Step 3 prior-run reconciliation prompt (only if the prior meta-block is unparseable, or for the drift gate when the manifest fingerprint changed); surface the Step 10 quality-gate failure prompt (Revise / Override / Restart); surface the Step 11 sub-step C Mermaid-validator failure / Gate-7 disagreement prompt; surface the Step 12 Accept / Revise / Restart prompt.
...
- The artefact contains exactly one `<section id="overview">`, one `<nav class="toc">`, one `<section id="source-roster">`, one `<section id="diagram">`, one `<section id="diagrams" class="object-board">`, one `<section id="tables">`, one `<section id="object-map-body">`, one `<details id="diagnostics" class="diagnostics-toggle">`, and one trailing `<section class="next-steps">`. DOM order is overview → toc → source-roster → diagram → diagrams → tables → object-map-body → diagnostics → next-steps.
...
- The `<section id="diagram">` contains exactly one `<details class="mermaid-block">` with a `<pre class="mermaid-source">`. The `<pre>` contains either a valid `erDiagram` declaration with ≥ 1 entity OR a `<!-- Mermaid erDiagram was rejected by the validator after 3 retries ... -->` comment (in which case Diagnostics records `[GAP-MERMAID-INVALID]`).
...
- All 8 quality-gate results are reported in the diagnostics block (either as `gate-pass` lines or as `gate-fail` lines with flagged items, with Gate 7 possibly `pass-with-drops`).
...
- Every consultant-supplied string in HTML body content is HTML-escaped (`<` → `&lt;`, `&` → `&amp;`, etc.). **Exception:** Mermaid syntax inside `<pre class="mermaid-source">` retains literal `|`, `[`, `]`, `{`, `}`, `||--o{`.
...
- `analyse-inputs/OOUX/ooux-object-map.html` exists, has been verified, and contains a complete OOUX object map: overview, source roster, Mermaid erDiagram (or documented drop), object column-board with ≥ 1 column, relationship matrix, machine-readable JSON body block, collapsed diagnostics with synonym-merge log + 8 gate results + irrelevant-to-domain rows + run history, and the Next-steps banner.
- Either all 8 hard quality gates passed (Gate 7 = pass or pass-with-drops), or the consultant explicitly chose Override and the Run-history bullet for this run records every violation.
- DOM order is overview → source-roster → diagram → diagrams → tables → object-map-body → diagnostics → next-steps.
...
- **Do not skip the Mermaid erDiagram.** The user's brief mandates "MUST contain a diagram". The graceful-degradation path (`[GAP-MERMAID-INVALID]`) is reserved for `mmdc` syntax failure after 3 validator retries — not for "I didn't bother". On mmdc unavailability the agent halts; it does not proceed without a diagram unless the validator explicitly degraded after retries.
