...
0. **In plain terms** (`<section id="plain-terms">`) — `{{PLAIN_SUMMARY}}`: the human-readable lead (2–5 plain-English sentences; faithful condensation; no new fact, count, or citation; no `[SRC]` of its own). The **first** section, above the meta-grid. Methodology jargon glossed at first use; client domain terms unglossed. Per `framework/shared/output-readability.md`.
...
2. **TOC** — sticky `<nav class="toc">` (template-owned); "In plain terms" is the first `<li>`.
3. **Diagrams** (`#diagrams`) — `{{THEME_MAP_SVG}}` (pre-rendered inline-SVG theme-map — the first **visual** after the plain-terms lead) **above** `{{THEME_MAP_MERMAID}}` (adjacent collapsed mermaid-source `<details>`).
...
9. **Downstream-use footer** — collapsed `<details class="downstream-toggle">` containing re-ingestion/markitdown guidance (machinery prose, for the consultant and pipeline only; not the reader's primary content).
10. **Footer** — credit (template-owned).
...
- For each row in `consumed_rows`, walk the content (source text, or the frozen description for converted-sibling rows) and produce **observations** — short factual notes pinned to verbatim sites:

  ```
  {
    observation_id,
    source_filename,
    excerpt: verbatim ≤ 200 chars (a lift from the frozen description for converted-sibling rows),
...
**D. Theme-map Mermaid source (`{{THEME_MAP_MERMAID}}`).** Emit the adjacent collapsed `<details class="mermaid-block">` with `<pre class="mermaid-source">` holding the `graph TD` source — the export / re-ingestion adjunct (survives markitdown HTML→MD as a fenced ```mermaid block). Build it exactly as before:
...
- The `<section id="plain-terms">` `<p>` contains at least 20 words and no `[SRC:` marker. It is a faithful condensation — it introduces no fact, count, or citation not present in the body.
- Methodology jargon is glossed at first use in the lead: if the lead uses the word "theme", "code", "inductive", "deductive", "code frequency", or "saturation", the gloss appears inline at first use (e.g. "themes (recurring patterns across the inputs)").
...
- The artefact contains, in order: `<section id="plain-terms">` first, then `#overview`, sticky `nav.toc`, a `#diagrams` section (with the inline-SVG theme-map `figure.theme-map` **above** the `details.mermaid-block`), a `#themes` section, a `#candidates` (Theme-to-requirement candidates) section, a `#coverage` section with `.covered` / `.gap` / `.silent` columns, the JSON body section `#thematic-analysis-body-section`, the collapsed `#diagnostics` `<details>` (containing the Consumed + Skipped source-roster tables and the run-history list), and the collapsed `details.downstream-toggle` footer.
