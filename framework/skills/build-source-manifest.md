...
**Purpose:** Emit `requirements/source-manifest.json` from the post-classification, post-conversion view of `input/`. The manifest is the drafter's sole input enumeration — the drafter does not glob `input/` and does not classify files. Every readable file the drafter will consume has a row here; unsupported files are recorded for forensic record but not read.
...
**Outputs:**
- `requirements/source-manifest.json` — the manifest file.
- A signal back to the input-handler indicating whether the manifest contains any non-Unsupported rows (relevant for `RF-03 input_no_supported_files`).

**Used by:**
- `framework/agents/requirements-input-handler.md` — called once after enumeration, classification, and the per-file conversion loop have completed.
...
- Do not write the manifest as markdown or any format other than the JSON schema above. The drafter parses with `Read` + JSON-decode; markdown tables are fragile to hand-editing.
- Do not omit Unsupported rows. The forensic record is the point — the consultant can see exactly what the drafter could not consume.
