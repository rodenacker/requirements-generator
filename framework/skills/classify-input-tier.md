...
- **Vector diagrams** — `.svg`, `.drawio`, and `.vsdx` are `Vector-renderable`. `.svg`/`.drawio` are XML text and `.vsdx` is a ZIP container (binary), but all three match the closed list in step 1, so the content sniff never runs for them — an `.svg` is therefore *not* mis-classified as `Native-text`, and a `.vsdx` is *not* mis-classified as `Unsupported`. The render-then-describe mechanism is gated by an external renderer; if the renderer is unavailable, the input-handler demotes these rows to `Unsupported` at conversion time (it does not happen here — this skill only assigns the tier).
