...
- `install-and-retry` — the agent halts Gate 9, surfaces the install advice path in the handback message, and exits cleanly so the consultant installs `mmdc` per `framework/shared/setup-instructions/mmdc.md` and re-invokes the pipeline. The agent may run `framework/tools/setup-environment.ps1 -Component mmdc` to install it on the consultant's behalf; the consultant restarts and re-invokes regardless (PATH cached at session start). The analyser does **not** write `framework/state/.progress.json` — the `/analyse-inputs` pipeline is bound by a no-write-outside-`analyse-inputs/` invariant (with the input-handler's documented manifest exception), and Step 10 has not yet written the artefact. Highest-fidelity outcome.
