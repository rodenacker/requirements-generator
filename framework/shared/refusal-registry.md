...
**Trigger:** A required external dependency — an **MCP-backed tool** or a **CLI binary** — is not available at preflight time. Three MVP cases: (a) `mcp__markitdown__convert_to_markdown` (probed via `framework/skills/preflight-mcp.md`) is required when at least one input file's tier is `Supported-via-MCP`; (b) a vector renderer (probed via `framework/skills/preflight-cli.md`) is required when at least one input file's tier is `Vector-renderable` **and renderer-gated** (`.svg` / `.vsdx`) — `.drawio` is **not** renderer-gated and falls back to XML decode (`framework/skills/decode-drawio-xml.md`) when the `drawio` binary is absent, so it never triggers this predicate; (c) a **Python 3 interpreter** (probed via `framework/skills/preflight-cli.md` with `binaries: ["python", "python3"]`) is required when a **Stadium 6 application** is ingested via the `/ingest-stadium` command (its `framework/agents/stadium-ingestor.md` agent) — the `framework/tools/extract_stadium_app.py` extractor is Python-stdlib-only, so it needs only the interpreter on PATH (no `pip install`). The predicate is dependency-agnostic; the surfacing caller supplies the `advice_path` of the specific dependency.

**Surface:** `AskUserQuestion` with the choice set `{ install-now, continue-skip, continue-later }` plus an "Other" override.
- `install-now` — the agent runs the matching component of the canonical installer, `framework/tools/setup-environment.ps1 -Component <X>` (`X` = `markitdown` for the markitdown case; `inkscape` for a `.svg` renderer, `libreoffice` for a `.vsdx` renderer; `python` for the Stadium case), reports the per-component result, then — because PATH and the MCP tool list are cached at session start — proceeds exactly as `continue-later` (writes `setup-pending` if `progress_path` is non-null, exits cleanly). The consultant restarts Claude Code and re-invokes; resumption finds the dependency present. This option only spares the consultant from typing the install command — it does **not** make the freshly-installed tool usable in the current session. (The Stadium case in `/ingest-stadium` re-probes `python` after the install and continues in-session if it now resolves, since the extractor runs as a fresh subprocess rather than a session-cached MCP server; if it still does not resolve, it falls back to this `continue-later` behaviour.)
...
- `continue-later` — the agent writes `status: setup-pending` and `pending_setup: { predicate: "RF-01", advice_path: "<the probed dependency's advice_path>", since: <ISO-8601 UTC> }` to the caller's progress file (`framework/state/.progress.json` for `/requirements`, `framework/state/.prd-progress.json` for `/generate-prd`; the `/analyse-inputs` and `/review-inputs` callers pass `progress_path: null` and simply exit cleanly; the `/ingest-stadium` command likewise has no progress file and simply exits cleanly on this branch) and exits cleanly. The consultant installs the dependency and re-invokes the pipeline. (The `advice_path` is `framework/shared/setup-instructions/markitdown.md` for the markitdown case, `framework/shared/setup-instructions/visual-render.md` for the vector-renderer case, `framework/shared/setup-instructions/stadium.md` for the Stadium-Python case.)

**Recovery:** Pick `install-now` to have the agent run the install for you (then restart + re-invoke), or read the `advice_path` surfaced in the question text — `framework/shared/setup-instructions/markitdown.md` (markitdown), `framework/shared/setup-instructions/visual-render.md` (vector renderer), or `framework/shared/setup-instructions/stadium.md` (Stadium Python) — and install manually.
...
- `install-and-retry` — the agent halts step-04, writes `status: setup-pending` and `pending_setup: { predicate: "RF-06", advice_path: "framework/shared/setup-instructions/playwright.md", since: <ISO-8601 UTC> }` semantics into the styler's own state (the styler does not write `framework/state/.progress.json` per the stand-alone constraint — it surfaces the install path in the handback message and exits cleanly so the consultant installs Playwright and re-runs `/design-system`). The agent may first run `framework/tools/setup-environment.ps1 -Component playwright` to warm the MCP server's `npx` cache on the consultant's behalf; the server still only registers on the next Claude Code start, so the consultant restarts and re-runs regardless. Highest-fidelity outcome.
...
**Trigger:** Gate 9 of an MVP analyser whose methodology emits Mermaid source requires `mmdc` (Mermaid CLI) on PATH to validate the emitted Mermaid `mindmap` or `flowchart TD` source via `framework/skills/mermaid-validator.md` (**both the skill and the `mmdc` dependency were retired — see the dormancy note below**). `mmdc` not found on PATH. Distinct from `RF-01` because the methodology-level rendering dependency is independent of the input-handler's markitdown MCP dependency, and distinct from the validator skill's own internal `not-installed` halt because the analyser exposes a three-way choice rather than a hard halt.

> **Fully dormant — no caller anywhere, and its dependencies were retired.** `affinity-mapping` was the original surfacer (its Step 10 Gate 9); it — and `swim-lane-process-mapping` — now **pre-render their diagrams as inline `<svg>`** and no longer validate Mermaid. The last remaining live caller, the `/requirements` drafter's §2.4 check, has since moved to a **dependency-free inline `mermaid-lint`** (no `mmdc`, no shell-out) — so this predicate now has no surfacing agent at all. In that migration the `framework/skills/mermaid-validator.md` skill, the `framework/shared/setup-instructions/mmdc.md` install doc, and the `mmdc` setup component were all **retired**. The entry remains defined (stable IDs are append-only — never renumber/delete) for any future Mermaid-render-dependent methodology that chooses to surface it; reviving it means re-creating a validator skill and installing the CLI with `npm install -g @mermaid-js/mermaid-cli`. The choice-option prose below is retained as a historical template and references those retired paths.
...
- `install-and-retry` — the agent halts Gate 9, surfaces the install command in the handback message, and exits cleanly so the consultant installs `mmdc` (`npm install -g @mermaid-js/mermaid-cli`) and re-invokes the pipeline. (The former `-Component mmdc` setup path and `setup-instructions/mmdc.md` doc were retired — see the dormancy note above; a reviving methodology would re-provide them.) The consultant restarts and re-invokes regardless (PATH cached at session start). The analyser does **not** write `framework/state/.progress.json` — the `/analyse-inputs` pipeline is bound by a no-write-outside-`analyse-inputs/` invariant (with the input-handler's documented manifest exception), and Step 10 has not yet written the artefact. Highest-fidelity outcome.
...
**Recovery:** `install-and-retry` exits cleanly so the consultant installs `mmdc` (`npm install -g @mermaid-js/mermaid-cli`) and re-invokes the pipeline. `skip-mermaid-validation-with-warning` continues the run with the diagnostics warning. `abort` exits cleanly with a failed handback.

**Refusal-registry schema field:** `setup_instructions_path` — same field as RF-01 and RF-06; its former target `framework/shared/setup-instructions/mmdc.md` was retired together with this predicate's last caller. A future surfacer would re-create an install doc (or inline `npm install -g @mermaid-js/mermaid-cli`).
...
- `install-and-retry` — the orchestrator writes `status: "setup-pending"` and `pending_setup: { predicate: "RF-10", advice_path: "framework/shared/setup-instructions/node-toolchain.md", since: <ISO-8601 UTC> }` to `framework/state/.prototype-progress.json` and exits cleanly. The orchestrator may first run `framework/tools/setup-environment.ps1 -Component node` to install Node on the consultant's behalf; PATH is cached at session start, so the consultant restarts Claude Code and re-invokes `/prototype` regardless; resumption picks up at scaffold.
...
- `install-and-retry` — the verify skill surfaces the advice path in its return; the orchestrator writes `status: "setup-pending"` + `pending_setup: { predicate: "RF-11", advice_path: "framework/shared/setup-instructions/playwright-browsers.md", since: <ISO-8601 UTC> }` to `framework/state/.prototype-progress.json` and exits cleanly. The generated route + spec remain on disk; re-invoke resumes at verify. Highest-assurance outcome. (This is **not** covered by `setup-environment.ps1` — the smoke browser is a system Chrome/Edge auto-detected by `playwright.config.ts`, or a one-off `npx playwright install chromium`, not a `-Component`; follow `playwright-browsers.md`.)
