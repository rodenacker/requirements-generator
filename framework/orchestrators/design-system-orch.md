...
- The styler requires interactive consultant input in the same thread — the step-02 URL prose prompt, the step-04b domain-suggestion menu (or its no-signals prose fallback), the step-04 RF-06 choice, and the step-07 accept/revise/restart loop (the menu, RF-06, and accept loop via `AskUserQuestion`) — none of which is surfaced in background harnesses.
...
- `Read` — check whether `design-system/design-system.html` (and the transition-window stale `design-system/design-system.md`) exists at startup. No other reads outside `design-system/` are permitted.
...
- Do not read, write, or edit `design-system/design-system.html` directly. The orchestrator's only direct disk operations are the existence check (Read) and the Reset procedure (Bash rm + git commit). Every other read or write belongs to the agent.
