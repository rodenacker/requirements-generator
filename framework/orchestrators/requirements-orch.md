...
Run the four requirements agents (input-handler → drafter → resolver → merger) in the prescribed order, gating each transition on an explicit handback from the agent that just ran, while logging progress to a file so a subsequent invocation can detect prior work and let the consultant either continue it or start fresh. Between the input-handler's handback and the drafter's invocation, ensure `manifest.target` is set: auto-set to `"prototype"` via `set-build-target.md` when null/absent; a non-null value (including legacy `"application"`) is honoured untouched.

Additionally: when Step 0 detects a **completed** run, offer the consultant `amend` as an alternative to regenerating. That branch delegates wholly to `framework/orchestrators/amend-requirements-orch.md` and exits before Step 0a — it runs none of the four agents and writes no progress state. This orchestrator is the discovery surface for amending (the consultant is already looking at a finished document at that moment); it is not the implementation, and the same pipeline is reachable directly as `/amend-requirements`.
...
- `status` is one of `"running" | "setup-pending" | "complete"`. Default is `"running"`. The orchestrator writes `"setup-pending"` when `RF-01 dependency_missing` fires with a `continue-later` choice; `"complete"` after the merger's `completed` event is written and its handback gate is satisfied. A progress file from before this schema bump that lacks a `status` field is treated as `"running"` (one-time additive migration; no rewrite required).
...
- **Event types** (all carry `t` = ISO-8601 UTC timestamp captured at the moment the event is written):
    ...
    - `{"t":"<iso>","type":"substep_start","stage":"drafter","substep":"<substep-name>","run_id":"<iso>"}` — written by **the drafter** immediately before each of its instrumented sub-steps. `<substep-name>` is one of `read-inputs`, `populate-template`, `gap-pass`, `derive-architectural-implications`, `author-mermaid`, `write-draft`, `write-claims-sidecar`, `grounding-verify`, `mermaid-lint`. `run_id` matches the current invocation's `run_start` `run_id` field — propagated by the drafter from in-thread context. The orchestrator does not write these events on the drafter's behalf; see `framework/agents/requirements-drafter.md > Timing log (sub-steps)` for the full sub-step schema, the start/end boundary mapping, and the paired-adjacent batching idiom.
...
2. **Classify.** Evaluate in this order — the first match wins.
    - **Completed run detected** — `requirements/requirements.md` exists on disk **and** `framework/state/.progress.json` contains a `completed` event for `requirements-merger`. The merger's `completed` event is the discriminator, **not** the presence of `requirements.md` and **not** `status = "complete"`: the merger seeds its output with `cp requirements/requirements-draft.md requirements/requirements.md` *before* its accept/edit/reject loop (`framework/agents/requirements-merger.md`), so an interrupted or rejected merge leaves the document on disk with `Status: draft` and no `completed` event. That case is an unfinished run and must classify as *Some progress detected* below, where `continue` resumes the merger.
    ...
    - **Some progress detected** — anything else (any event present, any artefact present, or both). If `status = "setup-pending"`, surface that state in the prompt text so the consultant knows the prior run paused on a refusal predicate and can resolve it before continuing.
3. **Prompt the consultant.** Use `AskUserQuestion` with the appropriate choice set:
    - **Completed run detected** — present a three-option prompt: `{ amend, regenerate, cancel }`. **`continue` is not offered** — every agent is complete, so there is nothing to resume and the option would resolve to a no-op. In the question text, state the document's `Status` and `Last finalised at` values, the count of `AMD-NN` entries already in its `## Amendments (pending re-merge)` section and how many `### Run ` sub-blocks they came from (both `0` when the section is absent), and one line on what each option costs. Extract all four values with a single bounded `Grep` of `requirements/requirements.md` — do not read the document body.
        - `amend the existing document (Recommended)` — *"State the changes you want; they are recorded as a new consultant-approved input document and applied as amendments to this document. Nothing is regenerated."*
        - `regenerate from the corpus` — *"Full re-draft: the six generated artefacts are checkpointed and deleted, the whole document is re-drafted from `input/`, and every `[AI-SUGGESTED]` marker is re-asked. Use this to fold pending amendments into the body with citations."*
        - `cancel` — *"Exit cleanly. Nothing is read, written, or deleted."*
...
4. **Branch on the consultant's choice.**
    - **amend** (only available when a completed run was detected) — invoke `framework/orchestrators/amend-requirements-orch.md` in the foreground, in this thread, and follow it verbatim (the same adopt-and-follow mechanic used for agents) until it reaches its Definition of Done. On its handback, append a `run_end` event to `framework/state/timing.ndjson` and **exit cleanly without entering Step 0a or any of Steps 1–4**.
        - Write **no** `.progress.json` events on this branch, and do **not** change `status` — it correctly remains `"complete"`. The amend pipeline has no agent in this orchestrator's event schema and its artefact is a dated `input/` filename the schema cannot express; the amend orchestrator writes no state of its own by design (see its **No progress file** section).
        - Do not run the Reset procedure, do not invoke the input-handler, and do not touch `requirements/source-manifest.json`. The new `input/` file is picked up by the next manifest create/refresh, exactly as with `/resolve-review`.
        - The `run_start` event for this invocation was already appended at Step 0; the closing `run_end` above is this orchestrator's responsibility because it opened the log entry, not the amend pipeline's.
    - **regenerate** (only available when a completed run was detected) — **identical in behaviour to `start-fresh`**: perform the **Reset procedure** below, then begin the pipeline at Step 1 (Step 0a fires per its own first-entry rule). The two choices differ only in label; the consultant reaching this one has a finished document, so "regenerate" describes the same action more honestly than "start fresh". Do not let the two diverge — a behavioural difference between them would be a defect, not a feature.
    - **cancel** (only available when a completed run was detected) — append a `run_end` event to `framework/state/timing.ndjson` and exit cleanly. No `.progress.json` write, no `status` change, zero deletions.
    - **continue** (only available when some progress was detected) — leave the progress file and artefacts in place. Resume the pipeline at the first agent whose `completed`-and-artefact pair is not satisfied. Do not re-run an agent whose work is already complete on disk and recorded in the progress file. **Exception for input-handler:** Step 1 (input-handler) always runs on every invocation, regardless of whether a `completed` event exists for it in the progress file. The input-handler owns the manifest-lifecycle decision (create / refresh / no-op / halt) at its own step 0, and the `no-op` path is a silent verify — so always running it on `continue` is the cheap way to catch input-folder drift between the original run and this rerun. The append-only events array absorbs the second `called` / `completed` pair without contradiction; the orchestrator does not skip Step 1 based on a prior `completed` event. (Step 1b — set build target — continues to honour its existing skip rule: skipped when `manifest.target` is already non-null on entry; the refresh path's `inherited_target` preservation keeps that rule's precondition intact across a rebuild.)
...
This procedure runs **only** when the consultant chose `start-fresh` (some progress detected) or `regenerate` (completed run detected) at startup — the two choices are the same action under different labels. If `start-fresh` was chosen with no prior progress, skip this procedure entirely — there is nothing to reset.

Note for the `regenerate` path: the checkpoint commit at step 1 stages `requirements/` and `input/`, so any `## Amendments (pending re-merge)` section about to be destroyed is preserved in git history — and its content also survives independently in the paired `input/amendments-<date>.md` (or `/resolve-review`) document, which step 4 never deletes (`framework/shared/input-safety.md > IS-01`). Folding those amendments into the regenerated body is the point of the re-draft.
...
- **After Input-handle:** the input-handler has handed control back when `requirements/source-manifest.json` exists, parses as JSON per the schema in `framework/skills/build-source-manifest.md`, contains at least one row with `tier ≠ "Unsupported"`, the agent's mode-specific self-validation has passed, and the manifest is in one of four accepted states:
    - `mode = "create"` or `mode = "refresh"` — the manifest was just written, was verified via `framework/skills/verify-artifact-write.md` (a `pass`), and the consultant has accepted it via the agent's handback prompt;
  ...
  Only then write the `completed` event for `input-handler`. If the agent fails its handback via `RF-01 continue-later`, `RF-03 abort`, step-0 `RF-04 manifest-corruption halt`, or step-0 `Cancel` at its drift prompt, do not write a `completed` event; the orchestrator stops cleanly.
...
- `framework/agents/input-handler.md` — invoked at Step 1 with `input_dir: "input/"`, `manifest_path: "requirements/source-manifest.json"`, `progress_path: "framework/state/.progress.json"`, **on every invocation** (the agent owns the create / refresh / no-op / halt decision; the orchestrator never branches on manifest state). Shared with `/generate-prd`, `/analyse-inputs`, `/review-inputs`.
- `framework/agents/requirements-drafter.md`
- `framework/agents/requirements-resolver.md`
- `framework/agents/requirements-merger.md`
- `framework/skills/set-build-target.md` — invoked at Step 1b to auto-set `target: "prototype"` in `requirements/source-manifest.json` when the field is `null` or absent.
- `framework/orchestrators/amend-requirements-orch.md` — invoked in the foreground on the Step-0 `amend` branch (completed-run state only), then this orchestrator appends `run_end` and exits. It owns every gate of the amend flow and writes no state; this orchestrator passes it no parameters (it performs its own bounded pre-flight).
- `requirements/requirements.md` — a **bounded** `Grep` at Step 0 in the completed-run state only: the header's `Status` / `Last finalised at` values plus the `AMD-\d+` and `### Run ` counts, for the prompt text. Never a body read. (Existence-only elsewhere, per **Startup: detect prior progress**.)
- `framework/shared/refusal-registry.md` — `RF-01`, `RF-03`, `RF-04` semantics surfaced by this orchestrator, by the input-handler, and by `set-build-target.md`.
...
- Grep — the Step-0 bounded extraction from `requirements/requirements.md` in the completed-run state only (header `Status` / `Last finalised at`, `AMD-\d+` count, `### Run ` count) to populate the prompt text. No other grep, and never a body read.
- AskUserQuestion — prompt the consultant at startup with the `{ start-fresh }`, `{ continue, start-fresh }`, or `{ amend, regenerate, cancel }` choice set as classified; and at Step 0a with the `{ continue, cancel }` choice set to gate input-handler invocation. (Step 1b prompts nothing — the build-target choice is retired; `target` is auto-set.)
...
The first six checks below apply on **every** branch. The remainder (Step 0a onward, the per-agent event and artefact assertions, and the `status: "complete"` check) apply only on the branches that actually run the four-agent pipeline — they are **not** evaluated on the Step-0 `amend` or `cancel` branches, which are documented clean exits that run no agent and produce no `requirements.md`.

- Step 0 (detect prior progress) ran and the consultant's choice was honoured (continued from the correct resume point, reset cleanly, delegated to the amend pipeline and exited, or cancelled cleanly).
- Step 0's classification used the **merger's `completed` event** — not the mere presence of `requirements/requirements.md`, and not `status = "complete"` — to distinguish a completed run from an interrupted or rejected merge. A document on disk with no merger `completed` event classified as *Some progress detected* and offered `continue`.
- On the `amend` branch: `framework/orchestrators/amend-requirements-orch.md` was invoked in the foreground and followed verbatim; Step 0a and Steps 1–4 did **not** run; the Reset procedure did **not** run; no `.progress.json` event was written and `status` was left `"complete"`; the input-handler was not invoked; `requirements/source-manifest.json` was neither read nor written; and a closing `run_end` event was appended to `framework/state/timing.ndjson` after the amend pipeline handed back.
- On the completed-run `cancel` branch: a `run_end` event was appended, nothing was written to `.progress.json`, and nothing was deleted.
- The completed-run prompt offered exactly `{ amend, regenerate, cancel }` — `continue` was **not** offered — and its text carried the document's `Status`, `Last finalised at`, and the two amendment counts, all extracted by a bounded `Grep` rather than a body read.
- Step 0a (input-ready prompt) ran on first entry, surfaced `AskUserQuestion` with `{ continue, cancel }`, honoured the consultant's choice (proceeded to Step 1 on `continue`; appended `run_end` to `framework/state/timing.ndjson` and exited cleanly on `cancel`), and was correctly skipped on a `continue` rerun where the input-handler already had a `completed` event (UX-only skip; Step 1 itself still ran).
...
- Step 1b (set build target) auto-set `target` to `"prototype"` on first entry after the input-handler's handback when `manifest.target` was `null` or absent (no consultant prompt, no timing events), and was correctly skipped when `manifest.target` was already non-null on entry. `requirements/source-manifest.json > target` is `"prototype"` or `"application"` before Step 2 is invoked.
...
**Pipeline run** (the `start-fresh`, `continue`, and `regenerate` branches):
...
**Amend branch:** `framework/orchestrators/amend-requirements-orch.md` reached its own Definition of Done (accepted run, clean exit, or a surfaced `RF-04` halt), this orchestrator surfaced that outcome, appended `run_end`, and exited without entering Steps 0a–4. No `.progress.json` write occurred and `status` is unchanged.

**Cancel branch** (completed-run state, or Step 0a `cancel`): a `run_end` event was appended and the invocation exited cleanly with nothing written to `.progress.json` and nothing deleted.
...
- Do not classify a completed run from the presence of `requirements/requirements.md` alone, or from `status = "complete"` alone. The merger `cp`-seeds the document **before** its accept loop, so an interrupted or rejected merge leaves it on disk with `Status: draft` and no `completed` event. Classifying that as a completed run would offer `amend` where `continue` is correct — stranding an unfinished merge and amending a document the consultant never accepted.
- Do not offer `continue` in the completed-run state. Every agent is complete, so it resolves to a no-op with no defined resume point.
- Do not enter Step 0a or any of Steps 1–4 on the `amend` branch. The amend pipeline writes its durable record into `input/`; continuing would let the input-handler refresh the manifest and the four agents re-draft the whole document over the amended corpus — a full regeneration, which is exactly what amending exists to avoid.
- Do not write `.progress.json` events, change `status`, or run the Reset procedure on the `amend` branch. The amend pipeline has no agent in this orchestrator's event schema, and `status` correctly remains `"complete"`.
- Do not omit the closing `run_end` event on the `amend` or completed-run `cancel` branches. This orchestrator appended the `run_start`, so it owns the close; the amend orchestrator writes no timing events by design.
- Do not re-implement any part of the amend flow here — not the intake, not the per-change asks, not the `input/` write, not the Amendments-section mechanics. Step 0's `amend` branch is a delegate-and-exit; `framework/orchestrators/amend-requirements-orch.md` owns every gate.
- Do not read the body of `requirements/requirements.md` at Step 0. The completed-run prompt's four values come from one bounded `Grep`; the full read belongs to the amend drafter.
...
- Do not skip Step 1b (set build target) when the manifest's `target` field is `null` or absent. The drafter and merger branch on this field; advancing to Step 2 with a null `target` corrupts the gap-pass's emission contract and the merger's PI append decision. Do not prompt the consultant for a build target — the choice is retired; `target` is auto-set to `"prototype"` (the application-audience document is produced by `/export-application`). Do not overwrite a non-null `target` on entry — the manifest is the durability anchor, and a legacy `"application"` value is honoured by the dormant downstream branches.
...
- Do not delete anything in `requirements/` other than the six named generated artefacts during a reset. Do not delete anything in `input/` other than `*.converted.md` siblings during a reset (`framework/shared/input-safety.md` `IS-03`). The progress file is overwritten with an empty events array, not deleted.
...
- Do not advance past `Input-handle` while `framework/state/.progress.json > status` is `"setup-pending"`. The status field is the orchestrator's halt signal; only the consultant's resume action (resolving the predicate and re-invoking) returns it to `"running"`.
