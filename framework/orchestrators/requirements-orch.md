...
- AskUserQuestion — prompt the consultant at startup with the `{ start-fresh }` or `{ continue, start-fresh }` choice set; and at Step 0a with the `{ continue, cancel }` choice set to gate input-handler invocation. (Step 1b prompts nothing — the build-target choice is retired; `target` is auto-set.)
