...
- **After Input-handle:** the input-handler has handed control back when `requirements/source-manifest.json` exists, parses as JSON, contains at least one row with `tier ≠ "Unsupported"`, has been verified via `framework/skills/verify-artifact-write.md`, the agent's self-validation has passed, and the consultant has accepted the manifest. If the agent fails its handback via `RF-01 continue-later` or `RF-03 abort`, do not write a `completed` event.
...
- `framework/agents/input-handler.md` — invoked at Step 1 with `input_dir: "input/"`, `manifest_path: "requirements/source-manifest.json"`, `progress_path: "framework/state/.prd-progress.json"`. Shared with `/requirements`, `/analyse-inputs`, `/review-inputs`.
