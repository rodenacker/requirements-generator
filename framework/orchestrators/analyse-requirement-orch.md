...
1. **Select methodology** — invoke `framework/skills/analysis-selector.md` (default labels; `registry_path: "framework/assets/analyses/registry.md"`). The skill reads the registry, filters `status == mvp`, prints a numbered list clustered by `group` (with `★ suggested next` / `✓ already run` marks), parses the consultant's typed reply, and returns one of `selected | cancelled | empty-registry`.
