...
- Do not skip step 1's input-handler invocation on a path that has not returned `cancelled` or `empty-registry` at step 0. Without it the manifest's freshness goes unchecked and (on first-mover) the manifest is never built; either way, the analyser has no usable source enumeration.
