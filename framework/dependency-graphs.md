...
- **Pure re-projection** of the finished `requirements/requirements.md` to the application audience: §6.10 fixtures → backend-contract pointers, §7 `prototype-fixture` → `backend-contract`, §0.1 `Target-mode applicability` → a fixed `## 0.1 Document scope` note, prototype-scope sentences elided from the five scope-note blockquotes (§1.7, §6.6.1, §6.6.2, §6.10, §7), PI appendix removed (heading→EOF — a contract `/resolve-review` depends on for Amendments placement), header `Target` flipped, `## Export provenance` block inserted (10 rows: source sha256 + citation legend + known residue + gate outcome). §1.7 / §6.6.1 / §6.6.2 + the §6.1 `Rationale` column pass through with only their prototype-scoping lead sentence elided — content and run-specific tails byte-identical. **Zero improvised content** — every net-new byte is mechanical or a fixed literal in the agent file.
