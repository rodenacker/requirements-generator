...
- `state/.progress.json` is read (existence + at-least-one-`completed`-event check) by `check-context-bloat.md` from the review-requirement orchestrator; the review-requirement orchestrator never writes to it, consistent with the no-write-outside-`reviews/` invariant.
