...
- The drafter's workflow has **ten** instrumented substeps in `framework/state/timing.ndjson` (was nine): `read-inputs`, `populate-template`, `grep-crosscheck`, `gap-pass`, `derive-architectural-implications`, `author-mermaid`, `write-draft`, `write-claims-sidecar`, `grounding-verify`, `mermaid-validate`. Total per clean drafter run: 11 tool calls for 20 events (was 10/18).
