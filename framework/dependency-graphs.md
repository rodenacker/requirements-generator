...
input-handler.md → check-manifest-freshness.md, classify-input-tier.md, preflight-mcp.md, preflight-cli.md,
                   ...
                   state/.stadium-processed.json [Step S: read-only, for the un-ingested nudge]
...
- drafter reads `stadium/asset-schemas.md` **only when** the scope includes `input/<AppName>.stadium-assets/*.md` rows (extracted by `/ingest-stadium`); it routes those assets' advisory / Tier-B lines to `[AI-SUGGESTED]`, never `[SRC]`.
...
prototype-app-scaffolder → scaffold-prototype-app.md, extract-brand-theme.md,
  ...
  scaffold-prototype-app → template/** (bulk copy on `copied`);
       ...
       `needs-install` — absent-only, never overwrite). ALSO invoked directly by
       prototype-orch Step F1's skip branch, which is the only call site on an
       already-scaffolded app and therefore the one that delivers new primitives.
...
- **Cross-pipeline reuse:** `scope-selector` (`propose_divergence_axes:false`) + `blueprint-architect` (`variants_output_path:null`, blueprint-only) + `design-philosophies.md` (the shared UX-posture registry, now under `framework/assets/wireframes/`). Write scope: `prototypes/**` + `framework/state/*` (+ the shared `blueprints/<slug>/{scope.json,blueprint.md}` exception via those two shared components). `prototypes/template-design-spec.md`, `ux-baseline-checklist.md`, `visual-craft-standard.md`, `scaffolding-instructions.md`, `app-shell-spec.md`, `shared-component-conventions.md` live under `framework/assets/prototypes/`; the posture registry `design-philosophies.md` lives under `framework/assets/wireframes/` (shared with `/wireframe`).
- **Visual-craft floor + device targets (2026-07-29).** `prototypes/visual-craft-standard.md` is the canonical floor for how a prototype *looks and feels* (token binding, press response, hover, overlay entrances, elevation ladder, typography, rhythm, focus, the six states) **and** the canonical owner of the named device viewports + per-target obligations (§11). Read by the drafter (§9 + §4b), the merger (§9 completion), and the generator (Inputs → `step-04`'s `visual_craft_subset` payload → `step-sub-render-surface` rules 2b/2c → `step-05` rule 3b → `step-07` static sweeps). `device_targets` is asked at `prototype-orch.md` **Step B(5)** (per prototype, always asked — unlike brand/colour-mode it is not scaffold-locked), stored in the design-spec front-matter + §4b, read at `step-02` rule 1, and projected onto Playwright projects by `verify-prototype-build.md`. Two **structural** enforcement points live in the template rather than in prose, because a global rule cannot be forgotten by one of eight parallel sub-agents: `template/src/app/globals.css` (the press layer, overlay entrances, reduced-motion collapse, base typography) and `template/e2e/craft.smoke.spec.ts` (app-level craft assertions, **copied not authored**, so single-mode apps get them too). `extract-brand-theme.md` now maps the **whole** design-system token set (type scale + weights + heading family + motion tiers + easing + the per-mode elevation ladder), not colour alone — 21 of 33 tokens were previously discarded, which is why generated components had no elevation, motion or type step to bind to.
- **Component minimum-feature contracts (2026-07-31).** `framework/assets/design-system-standards.md §1` — previously read by **no** prototype-pipeline file, while §2/§3/§4 were imported by `visual-craft-standard.md` — is now surfaced through `prototypes/ux-baseline-checklist.md` (projected there as **C1–C10**; §1 keeps sole ownership of the definitions). New edge chain: `design-system-standards.md §1` → `prototypes/ux-baseline-checklist.md` → {`prototype-spec-drafter` (§9 contracts + exceptions tables), `prototype-spec-merger` (§9 completion; exceptions passed through untouched), `step-02-read-spec` (reads both tables into the render plan), `step-04` (`component_contracts` payload per surface), `step-sub-render-surface` rule 5a, `step-05` rule 3c, `step-07` (mechanical sweep pattern 8 + the spec↔code pagination check), `verify-prototype-build.md` (per-table runtime assertions)}. It lands in the **baseline** floor, not the craft one, by the two files' own division of labour: §1 is whether a surface *works*. The one **structural** enforcement point is `template/src/components/ui/pagination.tsx` — shipped, so the `role="navigation"` landmark the smoke asserts is correct by construction instead of depending on eight parallel sub-agents stamping the same `aria-label`. Exceptions are per-surface + per-item + reasoned rows in design-spec §9, surfaced as `component_contract_warnings[]` in the generator handback and printed at `prototype-orch` Step G. `design-system-standards.md §5`/`§6` remain **owned but unconsumed** by this pipeline, recorded as such in `visual-craft-standard.md`'s canonical-owner table.
- **Clickable data-table rows are forbidden (2026-07-31).** `design-system-standards.md §1 > Tables` wins over the five framework + two template files that previously treated whole-row click as supported. `tr[data-clickable]`/`tr[data-pressable]` support stays in `template/{src/components/ui/table.tsx, src/app/globals.css}` for row-*like* lists that are not data tables (`pattern-catalogue/collections/data-list.md` is the protected carve-out); `pattern-catalogue/surfaces/drawer-detail.md` and `shared-component-conventions.md §7.1 inline-expand` now name a dedicated cell control as the trigger on a table host, since after this change a row-triggered fold would have no legal form.
...
- **Verify gate:** `verify-prototype-build.md` runs lint + tsc + Playwright smoke. The smoke visits **every** route the prototype authored (`routes`, one `test()` each — status, `proto-chrome`, zero console errors, plus the per-route colour-mode toggle assertions), not just `/<slug>`; the both-modes contrast sweep stays scoped to the primary route by design. New refusals `RF-10` (node missing), `RF-11` (playwright browsers), `RF-12` (build failed after retries), `RF-13` (scaffold failed). Invariants PI-01..PI-08 (PI-08 = chrome is a harness).
- **One nav table (2026-07-29):** intra-prototype nav is generated data — `src/data/nav/<slug>.ts` + an additive `src/data/nav/index.ts` barrel (scaffold-authored empty), schema canonical in `shared-component-conventions.md §6a`. Written at step-03 **before** dispatch so sub-agent-authored shells can import it; consumed at runtime via `NAV_BY_PROTOTYPE[<first path segment>]` because application shells are *shared* components. Distinct from `prototype-registry.ts` (inter-prototype, one row per prototype, regenerated after the gate). Role grants resolve **once** at step-02 rule 1b from `requirements.md` §6.5 with the transient amendments section applied — the design spec has no §6.5, and that dangling citation is why a real run shipped three competing nav derivations and two grant tables. `src/lib/` is closed to the generator; stores are one-per-§8-entity and no others.
...
## 12. ingest-stadium-orch.md · 9 nodes / 11 edges / depth 3

```
orch → stadium-ingestor, context-hygiene, refusal-registry,
       verify-artifact-write [re-ingest reset: ledger re-write],
       state/.stadium-processed.json [read: startup partition + re-ingest reset]
stadium-ingestor → extract-stadium-app, preflight-cli, verify-artifact-write, refusal-registry,
       setup-instructions/stadium.md, state/.stadium-processed.json [read/write: ledger]
...
- **The relocated Step S.** The `stadium-ingestor` agent is the former input-handler Step-S *extraction* logic, now first-class: detect Stadium units in `input/`, skip already-ledgered apps, preflight `python` (`preflight-cli`, `RF-01` → `setup-instructions/stadium.md`), invoke `extract-stadium-app` (→ `tools/extract_stadium_app.py`, `assets/stadium/*` via `--kb`), and write `state/.stadium-processed.json` keyed by `app_id` (+ the forensic `state/stadium/<app-id>/model.json` via the skill). It writes **no** source manifest — the produced `input/<AppName>.stadium-assets/*.md` assets are enumerated as ordinary `Native-text` rows by the next input-handler run.
- **Re-ingest gate + reset (orchestrator-owned).** At startup the orch scans `input/*.stadium` + `input/*/administration.db`, reads the ledger, and for each already-ledgered app surfaces `{ Skip, Re-ingest, Cancel }`. On `Re-ingest` it git-checkpoints then deletes the app's `input/<AppName>.stadium-assets/` + `state/stadium/<app-id>/` + its ledger entry (re-written via `verify-artifact-write`), so the agent re-extracts it as new — the same "delete-so-the-agent-redoes-it" reset shape as design-system's overwrite.
- **Write scope:** `input/<AppName>.stadium-assets/**`, `state/.stadium-processed.json`, `state/stadium/**`. Never `requirements/`. The input-handler's Step S separately excludes the raw app folder/pointer from manifest enumeration + the freshness check and nudges when a dropped app is un-ingested (see `@input-handler-subtree`).

---

## 13. amend-requirements-orch.md · 10 nodes / 13 edges / depth 3
...
- **Write scope:** `amend-requirements/amendments-draft.md` (transient staging, deleted on accept) + one NEW `input/amendments-<date>[-N].md` per accepted run (the **additive-`input/` write exception**, shared with `/resolve-review`) + the **Amendments-section write exception** via `apply-amendments-section` (unconditional here — no opt-in ask, because applying the projection is the pipeline's purpose; `no-question-when-the-outcome-is-determined`). Always after — and as a strict subset of — the verified `input/` write (pairing invariant). Never `framework/state/`.
...
- The input-handler is **never** invoked; pickup of the new `input/` file is the next manifest create/refresh's job. Context-load management is a step-0 >300 KB size advisory.
- **No new `RF-NN`.** `RF-04` covers all three writes (via `verify-artifact-write`, directly and inside the section skill); the missing-source and cancel paths are friendly non-refusal exits.
