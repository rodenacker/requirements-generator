...
|---|---|---|
| Agent | Persona+workflow `.md` the LLM adopts to produce one content artefact. | `CLAUDE.md §2` |
...
| Provenance markers | Closed tag set: `[AI-SUGGESTED]` `[STANDARD-RULE]` `[OUT-OF-SCOPE]` `[POSTURE-DEFAULT]`. | `CLAUDE.md §3` |
...
| Shared | Cross-pipeline invariants under `framework/shared/`; append-only. | `CLAUDE.md §2` |
| Sidecar | Compact machine-readable companion to a prose artefact (claims/projection JSON). | `analyses/sidecar-schema.md` |
| Skill | Reusable parameterised unit of agent behaviour returning a structured result. | `CLAUDE.md §2` |
