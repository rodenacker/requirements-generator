...
- **Recommendation** is one sentence proposing a concrete *corpus-handling* action — what `/requirements` and downstream consumers must do given this defect of the voice. The corpus IS the voice; the reviewer does not propose new elicitation. Five sanctioned forms, exhaustive:
    ...
    5. **Resolve at draft time** *(disposition: `Defer` — it resolves downstream without blocking the draft; see *Disposition rubric > Disposition → Recommendation coupling*)* — surface the defect in the consultant-answers loop during `/requirements` draft for explicit consultant choice. Reserved for load-bearing ambiguities (Dim 3) where downstream cannot apply a default. *"Resolve at draft time — surface 'real-time reporting' to consultant-answers; latency budget is load-bearing and cannot be defaulted."* This is a Recommendation form, **not** a disposition value — it never appears in the `disposition` field.
...
- The defect is **real** and **specific**, but downstream can handle it without consultant in-corpus reconciliation — typically by applying a `GR-NN` default, marking `[OUT-OF-SCOPE: domain-default]`, surfacing as `[AI-SUGGESTED]`, treating a role-claim as second-hand, or surfacing the defect at draft time in the `/requirements` consultant-answers loop.
- Common pattern: a missing edge-case workflow that affects a post-MVP feature (Recommendation form *Treat as silence*); a role with no corpus material that only matters in phase 2 (*Treat as silence*); a load-bearing second-hand claim for a phase-2 persona (*Treat as second-hand*); an integration whose contract belongs to a later phase (*Treat as silence*); a load-bearing ambiguity the consultant-answers loop can resolve at draft time (*Resolve at draft time*).
...
- Under the *corpus IS the voice* principle, Reject is reserved for three concrete patterns:
    ...
    3. **Load-bearing ambiguity unresolvable at draft time** (Dim 3). An ambiguous claim that downstream consultant-answers cannot resolve without external information (which the principle forbids), AND for which no `GR-NN` default can substitute. Rare — most ambiguities are `Defer` (Recommendation form *Resolve at draft time*: the consultant-answers loop resolves them without blocking the draft); only the genuinely *unresolvable* case is `Reject`, and its Recommendation form is *Reconcile in-corpus* (the consultant must reconcile the ambiguous claim in-corpus before drafting). See *Disposition rubric > Disposition → Recommendation coupling*.
...
### Disposition → Recommendation coupling

`Disposition` and `Recommendation` are **two separate fields**. The `disposition` field holds **only** the bare enum token (`Patch`, `Defer`, or `Reject`) — never a Recommendation phrase. The `recommendation` field holds exactly one of the five sanctioned forms (see *Finding schema > Recommendation*). Each disposition couples to a fixed subset of those forms:

| Disposition | Coupled Recommendation form(s) |
|---|---|
| `Patch`  | *Label / annotate* |
| `Defer`  | *Treat as silence* · *Treat as second-hand* · *Resolve at draft time* |
| `Reject` | *Reconcile in-corpus* |

Read the coupling in this direction: pick the disposition from the rubric above (by defect cost), then the `recommendation` field must carry one of *that* disposition's coupled forms. This is the single canonical statement of the coupling; every other passage (the recalibration section, the worker contract) references it rather than restating the mapping.

- ***Resolve at draft time* is a `Defer`, not a `Reject`.** Surfacing a load-bearing ambiguity to the `/requirements` consultant-answers loop *resolves* it downstream without blocking the draft — so its disposition is `Defer`. `Reject` is reserved for ambiguity that is *unresolvable* at draft time (Reject pattern 3), whose Recommendation form is *Reconcile in-corpus* — the consultant must reconcile the corpus before `/requirements` can draft at all.
- The literal string *"Resolve at draft time"* is an imperative phrase that can read like a disposition verb. **It is not a disposition.** If it — or any other Recommendation form — appears in a `disposition` field, the finding is malformed and fails quality gate 4 (`disposition` must be exactly one of the three enum tokens).
