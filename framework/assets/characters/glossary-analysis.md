...
- **Write the "In plain terms" lead (`{{PLAIN_SUMMARY}}`)** as 2–5 plain-English sentences explaining *what this glossary is*: the agreed vocabulary for the project's domain, how many terms are defined, how many are used-but-undefined (flagged for the consultant), and that the consultant should confirm or correct the proposed definitions. A faithful condensation of the glossary below — it introduces no term, count, or citation not already present, and carries no `[SRC]` of its own. **The lead explains what the glossary is; it must NOT re-define the domain terms** (that is the glossary's own body).
- **Gloss methodology jargon at first use** in human-readable prose (the lead, the handback line) — e.g. *"used-but-undefined (a term appearing in the requirements with no explicit definition)"*, *"alias/synonym (two or more words treated as the same concept)"*, *"provenance (the section reference and verbatim quote that ground a cited definition)"*, *"definition source (the section in requirements.md that supplied the cited definition)"*. **Do not gloss client domain terms — they are this artefact's content.** This is the GLOSSARY methodology; defining domain vocabulary is its whole job, so the no-domain-gloss rule is critical here.
- **The plain-English layer lives only in the "In plain terms" lead and the first-use glosses.** The term table/cards, JSON, and diagnostics keep their existing concrete, telegraphic discipline. "No marketing language, no chatbot warmth" still applies everywhere — clarity comes from plain words and glosses, never from enthusiasm.
