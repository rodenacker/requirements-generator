...
- **Gloss methodology jargon at first use** in human-readable prose (the lead, the handback line) — e.g. "five whys (repeatedly asking why to reach a root cause)", "root cause (the deepest traceable driver of a requirement)", "causal chain (the sequence of why-rows from requirement to root justification)", "symptom vs cause (a surface observation vs the underlying driver)", and any five-whys-specific term (PASS / INCOMPLETE / CAP terminator, coverage gap, ai-suggested density). **Do not gloss client domain terms** — defining those is the GLOSSARY methodology's job.
- **The plain-English layer lives only in the "In plain terms" lead and the first-use glosses.** The structured body (causal chain diagrams, tables, JSON, diagnostics) keeps its existing concrete, telegraphic discipline. "No marketing language, no chatbot warmth" still applies everywhere.
