...
2. Do **not** write `analyse-inputs/OPPORTUNITY-SOLUTION-TREES/opportunity-solution-tree.html`.
...
Re-runs **add to** the prior `analyse-inputs/OPPORTUNITY-SOLUTION-TREES/opportunity-solution-tree.html`; they do not replace it. The contract:
...
Optionally it re-reads the prior `analyse-inputs/OPPORTUNITY-SOLUTION-TREES/opportunity-solution-tree.html` for the additive merge.

The agent's only inputs are: the manifest, the per-row source files, this character file, the methodology reference, the HTML template (`framework/assets/analyses-inputs/template-opportunity-solution-trees.html`), and (optionally) the prior OST artefact. The analyser substitutes the template to produce a self-contained HTML artefact whose `#diagrams` section renders the discovery tree as a pre-rendered inline SVG with an adjacent collapsed Mermaid-source `<details>`, and which embeds a `language-json` `opportunity-solution-tree-body` block (tree model + candidate-requirement seeds) that survives a markitdown HTML→MD round-trip for re-ingestion.
...
The layered tree diagram is a pre-rendered, self-contained inline `<svg class="tree-svg">` — nodes + edges in one `viewBox` coordinate space, no `mmdc` / Mermaid-render dependency; the `graph TD` source beneath it is an unvalidated export adjunct, so there is no Mermaid-validation halt path.
