...
- **Relationships (§4.3):** the `_getFieldTypeName` `customTypes` map resolves **both cardinalities** — a value that is a `_..._Type extends Array` wrapper → `_getItemTypeName()` gives a **to-many** edge (`UserRead.Roles → Role[]`); a value that is a **direct `_..._Types_<Entity>` class** → a **to-one** edge (`Order.Customer → Customer`). Both feed §2.2 with an explicit cardinality. (Relational apps additionally get **SQL JOIN edges** — Tier-A — and **FK-by-naming-convention edges** incl. role-qualified suffix FKs like `CreditControllerUserId → User` — Tier-B `[AI-SUGGESTED]`; see `asset-schemas.md` §(f).)
