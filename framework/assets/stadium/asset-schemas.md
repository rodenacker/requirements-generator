...
| `surfaces` | The **view / task / feature inventory (0b)** (page · start? · design-surface? · reachable? · inferred kind); per-page control trees (layout + controls + visible text); the **action-affordance inventory → candidate tasks (0e)** (control · label · wired script → candidate task); templates; best-effort screen↔entity guesses | §6.4 (surfaces) + §5/screen-classification (inventory, affordances→tasks) + `/wireframe` (surface decomposition) |
