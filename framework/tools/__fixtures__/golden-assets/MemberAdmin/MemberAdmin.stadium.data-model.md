...
- `Members.UpdateDateTime` [from connector: MemberInsert]
  - relation (to-one (FK)): `Members.CityID` → `Cities` [from design model] `[AI-SUGGESTED: FK by naming convention]`
  - relation (join): `Members.CityID=ID` → `Cities` [from connector: MembersSelect]
