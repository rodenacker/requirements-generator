---
stadium_asset: glossary
app: RMB_Onboarding
file_guid: 87ea91de-6125-4686-8437-806216cb0ec0
designer_version: 6.14.3378.13771
selected_package: 3f1ddf96-3519-47b8-905d-b44703776f78.sapz
deployment_count: 1
last_published: 2026-06-30 12:19:12.1887333
extracted_from: C:\Stadium 6 Web Apps\87ea91de-6125-4686-8437-806216cb0ec0
provenance: deterministic extraction from the Stadium 6 design model + administration.db
marker_legend: Tier-A lines are authoritative facts ([SRC]-quotable); Tier-B lines are advisory design signals.
---
# Key terms (from page content) — RMB_Onboarding

## Tier-A — visible terms (verbatim from labels, headings, buttons, titles)

- 1202202210000001019
- 13
- A and B Signatories
- Abbreviated Name
- Accept
- Account Name
- Account Number
- Accounts
- Add
- Add Admin
- Add Administrator
- Add Entity
- Add Entity Details
- Add Signatory
- Add Sub Group
- Administrator ID
- Administrators
- Allocated
- Application Number
- Applications Dashboard
- Appointment of Administrators
- Audit log
- Back
- Branch Code
- CCN Number 1
- CCN Number 2
- CCN Number 3
- CCN Number 4
- CCN Number 5
- CCN Number 6
- Capacity
- Cashman
- Cashman Reports
- Client Applications
- Client Details
- Client Information
- Client Name
- Client Name:
- Client Number
- Client Registration Number:
- Client Segment:
- Client Service Request
- Client applications
- Close
- Collections
- Collections Application
- Company Name
- Complete the section below if applicable to Hierarchy
- Completed Applications
- Contact Email
- Contact Name
- Contact Person ID
- Contact Tel No
- Create Sub Group 1
- Create Sub Group 2
- Daily Average
- Designated Persons
- Designation
- Electronic Banking
- Electronic Banking Service Request
- Email address:
- Entity Name
- Entity Number
- Existing Client
- FTP Delivery Mechanism
- Full Name
- Full Name and Surname
- Generate
- Global Banking
- ID/Passport
- Identification Type
- Identity
- Joint
- Link Account
- Link CCN Number?
- Link an account below:
- Modular Information
- NAEDO
- New Client
- Next
- Nominated Account Payments
- Once off Payments
- Online Settlement Limits
- Pg Signatories
- Previous
- Reject
- Remove
- Remove Sub Group
- Save
- Service
- Severally
- Signatories
- Signatories Needed
- Signing arrangements
- Start Page
- Transfers
- Update
- View
