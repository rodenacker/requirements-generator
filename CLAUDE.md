...
**What.** Consultant-driven Claude Code workspace. Nine slash commands — each a prompt-only pipeline of markdown orchestrators + agents + skills — turn loose client material into structured artefacts. No runtime code: every "agent" is an `.md` file Claude reads and adopts as persona.
...
| `/wireframe` | 2–3 parallel low-fi HTML wireframe variants for a scope of `requirements/requirements.md`. Persona-bound; positioned on canonical UX trade-off dimensions; full requirement-ID traceability via `data-src` + `data-prop`; metadata-only `index.html` landing with side-by-side screen links. Cross-pipeline `blueprint-architect` agent + `scope-selector` skill reusable by a future `/prototype`. |
...
- Output mode = **prototype** (client-stub simulated server, fixture data — see `framework/shared/prototype-invariants.md` PI-01..PI-05) OR **application** (full backend reqs). Chosen once per manifest; durable in `requirements/source-manifest.json > target`.
...
- **Wireframe pipeline never invents object properties.** Every data-bound element (form input, table column, detail row, status chip) carries a `data-prop` naming a §7 data-shape property (`Shape.Field`) or F-NN-named parameter (`F-NN:ParamName`). The blueprint's per-screen `Properties` column is the canonical closed set; properties outside that set are fabrications and a `RF-04`-class self-validation FAIL. UI-only controls (search, sort, pagination, filter chips, save/cancel, dropzones) are exempt. `data-src="F-NN"` alone is insufficient justification — the field's property must be in the blueprint's closed set.
...
| `framework/agents/` | Agents = `.md` persona+workflow files. Cross-pipeline agents have no pipeline prefix (`input-handler.md` — shared by `/requirements`, `/generate-prd`, `/analyse-inputs`, `/review-inputs`; **single owner of the source-manifest lifecycle**; `blueprint-architect.md` + subdir — shared by `/wireframe` and a future `/prototype`; **single owner of the blueprint IR**, no pattern bindings). Pipeline-private agents use `<pipeline>-<role>.md` (`requirements-{drafter,resolver,merger}`, `prd-{drafter,resolver,merger}`, `design-system-styler/`, `wireframe-variant-generator/`, `wireframe-comparator.md`). Methodology agents nest under their pipeline: `agents/{analyses,analyses-inputs,reviews,reviews-inputs}/<method>-(analyser\|reviewer).md`. |
| `framework/skills/` | Reusable units of agent behaviour. Caller-agnostic, parameterised. Examples: `verify-artifact-write`, `check-context-bloat`, `check-manifest-freshness`, `classify-input-tier`, `convert-input-file`, `build-source-manifest`, `completeness-gap-pass`, `grounding-verifier`, `mermaid-validator`, `analysis-selector` (drives `/analyse-requirement`, `/analyse-inputs`, `/review-inputs`), `review-selector` (`/review-requirement` only), `set-build-target`, `scope-selector` (cross-pipeline — `/wireframe` + future `/prototype`), `select-supporting-analyses` (wireframe-private — on-disk-presence filter is load-bearing), `check-pattern-coverage`, `check-wireframe-set-freshness`. Plus `map-<method>-to-ui.md` per analysis methodology. |
| `framework/assets/` | Read-only reference content: `template-*.md/html`, `templates/` (cross-pipeline DS-agnostic), `design-systems/` (sibling DS files — currently `wireframe-ds.html`), `wireframes/` (wireframe-private templates + `tradeoff-dimensions-registry.md`, `pattern-bindings.md`, `domain-defaults.md`, `position-vocabulary.md`), `topics-*.md` (bijection invariants), `taxonomy-*.md`, `glossary.md`, `persona-llm.md`, `constraints.md`, `pattern-catalogue/`, `characters/*.md`, `analyses/registry.md` + per-method reference/template, `analyses-inputs/registry.md` + per-method, `reviews/registry.md` + per-method, `reviews-inputs/registry.md` + per-method, `references/`. |
| `framework/shared/` | Cross-pipeline invariants: `general-rules.md` (`GR-NN`), `prototype-scope.md` + `.index.md`, `prototype-invariants.md` (`PI-NN`), `refusal-registry.md` (`RF-NN`), `setup-instructions/{markitdown,playwright}.md`. |
| `framework/state/` | Runtime state. `.progress.json` (requirements-orch only), `.prd-progress.json` (generate-prd-orch only — same shape), `timing.ndjson` (append-only across all runs), `draft-fragments/`, resolver sidecars per pipeline. `/analyse-inputs`, `/review-inputs`, `/wireframe`, `/design-system`, `/analyse-requirement`, `/review-requirement` do **not** own a progress file. |
...
| `blueprints/<scope-slug>/` | **Shared cross-pipeline IR root**, created by `/wireframe` and reusable by a future `/prototype` without re-authoring. Per-scope: `scope.json` (which requirement IDs are in scope) + `blueprint.md` (screen inventory + flow + scope→screen trace + per-screen Properties closed set drawn from §7 data shapes and F-NN parameters; **no pattern bindings** — those live per-variant). |
| `wireframes/<scope-slug>/` | `/wireframe` outputs (wireframe-private): `analyses-inputs.json` (Stage-1b consultant selection of completed-on-disk supporting analyses — schema in `framework/skills/select-supporting-analyses.md`), `variants.json` (architect's variant configs — persona-bound, cardinality cap 3), `index.html` (metadata-only consultant landing, four sections: scope details / side-by-side wireframe columns / variant metadata cards / trade-off matrix), `_drift.json` (system file; index surfaces a one-line summary), and per-variant `<VARIANT>/` subdirs (`screen-NN-*.html` with `data-src` + `data-prop` audit attributes, `wireframe-ds.css`, `manifest.json`, `variant-position.json`). |
...
- **`blueprint-architect.md` + `scope-selector.md`** (shared by `/wireframe` and future `/prototype`) write `blueprints/<scope-slug>/{scope.json, blueprint.md}`.
...
| New `/analyse-requirement` methodology | `framework/agents/analyses/<method>-analyser.md` + reference + template + character + map skill + sidecar emission per `framework/assets/analyses/sidecar-schema.md` | Append row to `framework/assets/analyses/registry.md` (status: `mvp`); append `architect_roles` row to `framework/skills/select-supporting-analyses.md`. Zero orch changes. Skipping sidecar emission trips `RF-09` legacy-fallback on selection in `/wireframe`. |
| New `/analyse-inputs` methodology | `framework/agents/analyses-inputs/<method>-analyser.md` + `framework/assets/analyses-inputs/<method>-reference.md` + template + character + map skill (or reuse `map-<method>-to-ui.md` if source-agnostic) | Promote row in `framework/assets/analyses-inputs/registry.md` from `future` to `mvp`, fill all eight fields. `output_path` under `analyse-inputs/<METHOD>/`. Zero orch changes. |
| New `/review-requirement` methodology | `framework/agents/reviews/<method>-reviewer.md` + reference + template + character | Append row to `framework/assets/reviews/registry.md` (status: `mvp`). Zero orch changes. |
| New `/review-inputs` methodology | `framework/agents/reviews-inputs/<method>-reviewer.md` + reference + template + character | Promote row in `framework/assets/reviews-inputs/registry.md`. `output_path` under `review-inputs/<METHOD>/`. Zero orch changes. |
...
| New wireframe variant configuration shape | Architect produces `variants.json`; generator produces per-variant sidecars — no registry. Configuration emerges per run from consultant choices. | If `variants.json` / `<VARIANT>/{manifest.json, variant-position.json}` shape evolves, update `blueprint-architect.md`, `wireframe-variant-generator.md`, `wireframe-comparator.md`, and `framework/assets/wireframes/template-set-index.html` together — implicit schema contract. |
| New cross-pipeline asset (shared by `/wireframe` + future `/prototype`) | `framework/assets/templates/` (DS-agnostic) OR `framework/assets/design-systems/` (sibling DS) OR `framework/assets/wireframes/` (wireframe-private) per scope | Reference from consuming agent. Cross-pipeline agents use no pipeline prefix (`input-handler.md`, `blueprint-architect.md`). |
...
Mechanically derived embedding (e.g. the merger appending `PI-01..05` verbatim from the canonical source into every `requirements.md`, or `[SRC: C-NNN]` markers copied from `draft-claims.ndjson` into `requirements-draft.md`) is **not** a violation. Restating constraints in an agent's *Self-validation* section for fail-closed resilience is also not a violation.
...
- **Pipeline outputs.** `<artefact>.md/.html/.json/.ndjson` under the pipeline's own dir. Analyses + reviews use UPPERCASE-METHOD subdirs (`analyse-requirements/OOUX/`, `review-inputs/COMPLETENESS-REVIEW/`).
- **Markers in content.** `[SRC: C-NNN]` (input-cited fact in `/requirements` draft, sidecar-backed by `draft-claims.ndjson`, stripped by merger). `[SRC: <filename>]` (filename-cited fact in `/analyse-inputs` and `/review-inputs` artefacts — manifest row's `filename` payload). `[AI-SUGGESTED: AI-NNN | blocking|non-blocking]` (drafter inference, resolver Q&A). `[STANDARD-RULE: GR-NN]` (deterministic, resolver skips). `[OUT-OF-SCOPE: domain-default]` (prototype-only, resolver skips). Stable-ID prefixes: `C-` claims, `AI-` suggestions, `PC-` / `PAI-` PRD equivalents, `GR-` general rules, `RF-` refusals, `PI-` prototype invariants.
