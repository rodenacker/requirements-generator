...
**Marker legend:**
- `[SRC: <id>]` — grounded in a requirement ID (`F-NN`/`BR-NN`/`UI-NN`/`§7.Shape`), a blueprint surface (`LS-NN`), a selected wireframe variant (`WF:<variant>`), or an explicit consultant directive (`consultant`). **Retained** by the merger as provenance.
- `[POSTURE-DEFAULT]` — value taken deterministically from the chosen posture preset (resolver skips; merger strips).
- `[AI-SUGGESTED: AI-NNN | blocking|non-blocking]` — drafter UX inference (resolver Q&A; merger strips).
...
| Field | Value |
|---|---|
| Posture | Analytical / Information-Dense [POSTURE-DEFAULT] |
...
_(Left for the merger to fill from `framework/shared/prototype-invariants.md` — PI-01..PI-08.)_
