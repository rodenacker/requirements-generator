...
- **Tool** = sanctioned executable helper under `framework/tools/` (e.g. `timing-report.mjs`, `setup-environment.ps1`). The system is otherwise prompt-only; tools are the narrow exception, invoked by a command/skill, and must never read or write pipeline artefacts. `setup-environment.ps1` is the **canonical home for every dependency install command**: `/setup`, the `RF-01/06/10/11` handbacks, and the `setup-instructions/*.md` docs reference it by component name (`-Component <x>`) rather than restating commands — a "mechanically derived embedding" under the *Canonical-source rule*, kept in sync with the troubleshooting docs.
