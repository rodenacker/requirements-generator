...
| New ingestion runtime-code helper (e.g. a new app-format extractor) | `framework/tools/<name>.{py,…}` — the sanctioned runtime-code location, alongside `extract_stadium_app.py` and the markitdown/inkscape ingestion precedents (and `/prototype`'s generator) | Invoke it from a skill that wraps it (e.g. `framework/skills/extract-stadium-app.md`); the helper does deterministic work only and must not read or write pipeline artefacts beyond its declared extraction outputs. The input-handler **Step-S pre-pass** detects/excludes the corresponding input *unit* from manifest enumeration and the freshness check. |
