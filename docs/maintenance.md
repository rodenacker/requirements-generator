...
- **Tool** = sanctioned executable helper under `framework/tools/` (e.g. `timing-report.mjs`, `setup-environment.ps1`). The system is otherwise prompt-only; tools are the narrow exception, invoked by a command/skill — **or by a harness hook declared in `.claude/settings.json`** — and must never write pipeline artefacts. A hook-invoked tool may **read** an artefact for a read-only consultant affordance (`open-artifact.cjs` stats + hashes the file it previews); it still writes nothing under a pipeline output dir, and it must fail open so it can never halt a pipeline. Note `.claude/helpers/` is **gitignored** (ruflo-owned — see `.gitignore`), so a framework-owned helper belongs in `framework/tools/` or it disappears on clone. `setup-environment.ps1` is the **canonical home for every dependency install command**: `/setup`, the `RF-01/06/10/11` handbacks, and the `setup-instructions/*.md` docs reference it by component name (`-Component <x>`) rather than restating commands — a "mechanically derived embedding" under the *Canonical-source rule*, kept in sync with the troubleshooting docs.

### Stand-alone constraints (write isolation)

Each pipeline writes **only to its own output dir**. The four **documented cross-pipeline exceptions** (the first two inherited from shared agents; the third and fourth each shared by `/resolve-review` and `/amend-requirements`):
...
- **The additive `input/` write** — `resolve-review-drafter.md` writes one NEW `input/<stem>-<date>.md` per accepted run; `amend-requirements-drafter.md` writes one NEW `input/amendments-<date>.md`. Both are **additive only**, never overwriting an existing `input/` file (same-day collisions suffix `-2`, `-3`), per `framework/shared/input-safety.md > IS-01`. Each pipeline stages its draft under its own output dir (`resolve-review/`, `amend-requirements/`) so an unaccepted document never enters `input/`.
- **The `## Amendments (pending re-merge)` section write** — owned by the shared skill **`framework/skills/apply-amendments-section.md`**, called by `resolve-review-drafter.md` Step 9b (review-requirements-sourced runs, consultant opt-in only) and `amend-requirements-drafter.md` Step 9 (unconditional — that projection is the pipeline's purpose). **Bounded to that one section** in `requirements/requirements.md` (placed before the PI appendix, since `/export-application` strips PI-heading→EOF; no other byte changes), always paired with — and a subset of — the `input/` document written first (the pairing invariant). The section is a transient cache: the next `/requirements` re-merge regenerates the doc and folds the same content in from `input/`. Canonical shape: `framework/assets/resolve-review/template-addendum.md`; the placement, `AMD-NN` numbering, byte-isolation and pairing mechanics live in the skill and are **not** restated in either caller.

  *A third caller must go through the skill.* Re-implementing the section write inline is the failure mode the extraction exists to prevent — two copies of the placement and numbering rules drift, and a section placed after the PI heading vanishes silently at export.
...
| New consumer of the transient `## Amendments (pending re-merge)` section | Read the section from `requirements/requirements.md` and apply its `AMD-NN` entries as supersessions over the base text they name (precedent: `blueprint-architect/steps/step-02-read-inputs.md`, which applies them to the §7/F-NN closed property set; `export-application-exporter.md`, which retains them byte-identical and decodes their markers) | Never *write* the section outside `framework/skills/apply-amendments-section.md`. Shape is canonical in `framework/assets/resolve-review/template-addendum.md` |
...
| New harness-level affordance (a trigger→response with no reasoning in it, that would otherwise need the same step inserted at many write sites) | `framework/tools/<name>.cjs` (**not** `.claude/helpers/` — gitignored) + a hook group in `.claude/settings.json`. Must fail open (always exit 0) and never write a pipeline artefact. The hook `command` must be **shell-agnostic**: a bare `node <repo-relative-path>.cjs`, never `cmd /c` (MSYS rewrites `/c` to `C:/` under Git Bash, so `cmd.exe` starts interactively, eats the stdin payload and exits 0) and never `%VAR%`/`$VAR` interpolation (neither form expands under both shells). Must ship a **payload-level self-test** (precedent: `open-artifact.cjs --selftest`) — a fail-open, exit-0 design makes a broken command string indistinguishable from success, so a probe that exercises the real transport is the only way to tell them apart | Canonical policy in `framework/shared/<noun>.md` (precedent: `artifact-preview.md`), referenced from `CLAUDE.md` + `docs/architecture.md`; note it in `framework/dependency-graphs.md` as out-of-graph. **Zero** orchestrator / agent / skill call sites — adding one would double-fire it |
...
| New ingestion runtime-code helper (e.g. a new app-format extractor) | `framework/tools/<name>.{py,…}` — the sanctioned runtime-code location, alongside `extract_stadium_app.py` and the markitdown/inkscape ingestion precedents (and `/prototype`'s generator) | Invoke it from a skill that wraps it (e.g. `framework/skills/extract-stadium-app.md`, invoked by `/ingest-stadium`'s `stadium-ingestor` agent); the helper does deterministic work only and must not read or write pipeline artefacts beyond its declared extraction outputs. Extraction is triggered by its own command (like `/ingest-stadium`); the input-handler **Step-S pre-pass** only detects/excludes the corresponding input *unit* from manifest enumeration and the freshness check, and nudges when the unit is un-ingested. |
